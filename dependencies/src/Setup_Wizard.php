 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\Barn2\Setup_Wizard;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\Barn2\Setup_Wizard\Interfaces\Bootable;
...
class Setup_Wizard implements Bootable
{
    ...
    public function register_admin_page()
    {
        ...
        $page_title = \sprintf(__('%s setup wizard', 'easy-post-types-fields'), $this->plugin->get_name());
    ...
    public function add_restart_btn()
    {
		...
		<div class="barn2-setup-wizard-restart">
			<hr>
			<h3><?php 
        esc_html_e('Setup wizard', 'easy-post-types-fields');
        ?></h3>
			<p><?php 
        esc_html_e('If you need to access the setup wizard again, please click on the button below.', 'easy-post-types-fields');
        ?></p>
			<a href="<?php 
        echo esc_url($url);
		...
		<script>
			jQuery( '.barn2-wiz-restart-btn' ).on( 'click', function( e ) {
				return confirm( '<?php 
        echo esc_html(\sprintf(__('Warning: This will overwrite your existing settings for %s. Are you sure you want to continue?', 'easy-post-types-fields'), $this->plugin->get_name()));
        ?>' );
			});
		</script>
		<?php 
    ...
    public function add_restart_link(string $wc_section_id, string $title_option_id)
    {
        add_filter("woocommerce_get_settings_{$wc_section_id}", function ($settings) use($title_option_id) {
            ...
            if ($title_setting && isset($title_setting[\key($title_setting)]['desc'])) {
                ...
                $new_desc = \substr_replace($desc, ' | <a class="barn2-wiz-restart-btn" href="' . esc_url($url) . '">' . esc_html__('Setup wizard', 'easy-post-types-fields') . '</a>', $p_closing_tag, 0);
