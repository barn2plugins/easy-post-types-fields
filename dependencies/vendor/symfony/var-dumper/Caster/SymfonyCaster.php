...
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\Symfony\Component\Uid\Ulid;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\Symfony\Component\Uid\Uuid;
...
class SymfonyCaster
{
    ...
    public static function castUuid(Uuid $uuid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($uuid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $uuid->getDateTime()->format('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\Y-m-d H:i:s.u \\U\\T\\C');
        }
        return $a;
    }
    public static function castUlid(Ulid $ulid, array $a, Stub $stub, bool $isNested)
    {
        ...
        if (\method_exists($ulid, 'getDateTime')) {
            $a[Caster::PREFIX_VIRTUAL . 'time'] = $ulid->getDateTime()->format('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\Y-m-d H:i:s.v \\U\\T\\C');
        }
        return $a;
    }
}
