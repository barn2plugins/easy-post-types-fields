    ...
    public function fromMethodReturnType(ReflectionMethod $method) : Type
    {
        ...
        if ($returnType instanceof ReflectionNamedType) {
            ...
            if ($returnType->getName() === 'static') {
                return new StaticType(TypeName::fromReflection($method->getDeclaringClass()), $returnType->allowsNull());
            }
            ...
            if ($returnType->getName() === 'parent') {
                ...
                if ($parentClass === \false) {
                    throw new RuntimeException(sprintf('%s::%s() has a "parent" return type declaration but %s does not have a parent class', $method->getDeclaringClass()->getName(), $method->getName(), $method->getDeclaringClass()->getName()));
                }
        ...
        foreach ($returnType->getTypes() as $type) {
            ...
            if ($type->getName() === 'self') {
                $types[] = ObjectType::fromName($method->getDeclaringClass()->getName(), \false);
            } else {
