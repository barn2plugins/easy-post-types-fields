...
abstract class AbstractNewFeatureSniff extends AbstractComplexVersionSniff
{
    ...
    {
        $errorInfo = array('not_in_version' => '', 'error' => \true);
        $versionArray = $this->getVersionArray($itemArray);
        if (empty($versionArray) === \false) {
            foreach ($versionArray as $version => $present) {
                if ($errorInfo['not_in_version'] === '' && $present === \false && $this->supportsBelow($version) === \true) {
    ...
    public function addError(File $phpcsFile, $stackPtr, array $itemInfo, array $errorInfo)
    {
        ...
        $data = array($itemName, $errorInfo['not_in_version']);
