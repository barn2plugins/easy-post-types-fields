 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\PHPCSHelper;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_Exception as PHPCS_Exception;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_File as File;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_Sniff as PHPCS_Sniff;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_Tokens as Tokens;
/**
...
abstract class Sniff implements PHPCS_Sniff
{
     ...
     * @var array
     */
    protected $superglobals = array('$GLOBALS' => \true, '$_SERVER' => \true, '$_GET' => \true, '$_POST' => \true, '$_FILES' => \true, '$_COOKIE' => \true, '$_SESSION' => \true, '$_REQUEST' => \true, '$_ENV' => \true);
    /**
     ...
     * @var array
     */
    protected $hashAlgoFunctions = array('hash_file' => 1, 'hash_hmac_file' => 1, 'hash_hmac' => 1, 'hash_init' => 1, 'hash_pbkdf2' => 1, 'hash' => 1);
    /**
     ...
     * @var array
     */
    protected $iniFunctions = array('ini_get' => 1, 'ini_set' => 1);
    /**
    ...
    private function getTestVersion()
    {
        ...
        $testVersion = \trim(PHPCSHelper::getConfigData('testVersion'));
        if (empty($testVersion) === \false && isset($arrTestVersions[$testVersion]) === \false) {
            ...
            if (\preg_match('`^(\\d+\\.\\d+)?\\s*-\\s*(\\d+\\.\\d+)?$`', $testVersion, $matches)) {
                if (empty($matches[1]) === \false || empty($matches[2]) === \false) {
                    ...
                    if (\version_compare($min, $max, '>')) {
                        \trigger_error("Invalid range in testVersion setting: '" . $testVersion . "'", \E_USER_WARNING);
                        return $default;
                    } else {
            ...
            \trigger_error("Invalid testVersion setting: '" . $testVersion . "'", \E_USER_WARNING);
            return $default;
        }
    ...
    public function supportsAbove($phpVersion)
    {
        ...
        if (\is_null($testVersion) || \version_compare($testVersion, $phpVersion) >= 0) {
            return \true;
        } else {
            return \false;
        }
    ...
    public function supportsBelow($phpVersion)
    {
        ...
        if (\is_null($testVersion) === \false && \version_compare($testVersion, $phpVersion) <= 0) {
            return \true;
        } else {
            return \false;
        }
    ...
    public function doesFunctionCallHaveParameters(File $phpcsFile, $stackPtr)
    {
        ...
        if (\in_array($tokens[$stackPtr]['code'], array(\T_STRING, \T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \T_VARIABLE), \true) === \false) {
            return \false;
        }
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        // Deal with short array syntax.
        if ($tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY) {
        ...
        if ($nextNonEmpty === \false && $tokens[$nextNonEmpty]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
            return \false;
        }
        ...
        $nextNextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, $closeParenthesis + 1, \true);
    ...
    public function getFunctionCallParameters(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY) {
            ...
            $opener = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $paramStart = $opener + 1;
        $cnt = 1;
        while (($nextComma = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, $tokens[$closer]['code'], \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE), $nextComma + 1, $closer + 1)) !== \false) {
            ...
            if ($tokens[$nextComma]['type'] === 'T_CLOSURE' && (isset($tokens[$nextComma]['scope_condition']) && $tokens[$nextComma]['scope_condition'] === $nextComma) && isset($tokens[$nextComma]['scope_closer'])) {
                // Skip forward to the end of the closure declaration.
                $nextComma = $tokens[$nextComma]['scope_closer'];
                continue;
            }
            // Ignore comma's at a lower nesting level.
            if ($tokens[$nextComma]['type'] === 'T_COMMA' && isset($tokens[$nextComma]['nested_parenthesis']) && \count($tokens[$nextComma]['nested_parenthesis']) !== $nestedParenthesisCount) {
                continue;
            }
            ...
            $parameters[$cnt]['raw'] = \trim($phpcsFile->getTokensAsString($paramStart, $nextComma - $paramStart));
            /*
             * Check if there are more tokens before the closing parenthesis.
             * Prevents code like the following from setting a third parameter:
             * `functionCall( $param1, $param2, );`.
             */
            $hasNextParam = $phpcsFile->findNext(Tokens::$emptyTokens, $nextComma + 1, $closer, \true, null, \true);
     ...
     */
    public function inClassScope(File $phpcsFile, $stackPtr, $strict = \true)
    {
        $validScopes = array(\T_CLASS);
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_ANON_CLASS') === \true) {
            $validScopes[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS;
        }
        if ($strict === \false) {
    ...
    public function getFQClassNameFromNewToken(File $phpcsFile, $stackPtr)
    {
        ...
        $start = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if ($start === \false) {
            return '';
        }
        ...
        $find = array(\T_NS_SEPARATOR, \T_STRING, \T_NAMESPACE, \T_WHITESPACE);
        $end = $phpcsFile->findNext($find, $start + 1, null, \true, null, \true);
    ...
    public function getFQExtendedClassName(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] !== \T_CLASS && $tokens[$stackPtr]['type'] !== 'T_ANON_CLASS' && $tokens[$stackPtr]['type'] !== 'T_INTERFACE') {
            return '';
        }
    ...
    public function getFQClassNameFromDoubleColonToken(File $phpcsFile, $stackPtr)
    {
        ...
        if (\in_array($tokens[$stackPtr - 1]['code'], array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT, \T_STATIC), \true)) {
            return '';
        }
        // Get the classname from the class declaration if self is used.
        if ($tokens[$stackPtr - 1]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF) {
        ...
        $find = array(\T_NS_SEPARATOR, \T_STRING, \T_NAMESPACE, \T_WHITESPACE);
        $start = $phpcsFile->findPrevious($find, $stackPtr - 1, null, \true, null, \true);
    ...
    public function getDeclaredNamespaceName(File $phpcsFile, $stackPtr)
    {
        ...
        $nextToken = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if ($tokens[$nextToken]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET) {
            /*
        ...
        $validTokens = array(\T_STRING => \true, \T_NS_SEPARATOR => \true, \T_WHITESPACE => \true);
        $namespaceName = '';
        while (isset($validTokens[$tokens[$nextToken]['code']]) === \true) {
            $namespaceName .= \trim($tokens[$nextToken]['content']);
            $nextToken++;
        }
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_RETURN_TYPE') && $tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_RETURN_TYPE) {
            return $stackPtr;
        }
        if ($tokens[$stackPtr]['code'] !== \T_FUNCTION && $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE) {
            return \false;
        }
        ...
        if (isset($tokens[$stackPtr]['scope_opener'])) {
            ...
            $nextSemiColon = $phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, $tokens[$stackPtr]['parenthesis_closer'] + 1, null, \false, null, \true);
        ...
        $hasColon = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE), $tokens[$stackPtr]['parenthesis_closer'] + 1, $endOfFunctionDeclaration);
        if ($hasColon === \false) {
            return \false;
        }
        /*
        ...
        $unrecognizedTypes = array(
            \T_CALLABLE,
            \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF,
            \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT,
            \T_ARRAY,
            // PHPCS < 2.4.0.
            \T_STRING,
        );
    ...
    public function getReturnTypeHintName(File $phpcsFile, $stackPtr)
    {
        ...
        $colon = $phpcsFile->findPrevious(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE, \T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS), $stackPtr - 1);
        if ($colon === \false || $tokens[$colon]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON && $tokens[$colon]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE) {
        ...
        for ($i = $colon + 1; $i <= $stackPtr; $i++) {
            ...
            if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_NULLABLE') === \false && $tokens[$i]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN) {
                // Old PHPCS.
                continue;
            }
    ...
    public function isClassProperty(File $phpcsFile, $stackPtr)
    {
        ...
        $validScopes = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_TRAIT' => \true);
        ...
        if ($scopePtr !== \false) {
            ...
            if (empty($tokens[$stackPtr]['nested_parenthesis']) === \true) {
                ...
                if ($deepestOpen < $scopePtr || isset($tokens[$deepestOpen]['parenthesis_owner']) === \false || $tokens[$tokens[$deepestOpen]['parenthesis_owner']]['code'] !== \T_FUNCTION) {
                    return \true;
                }
            }
        }
    ...
    public function isClassConstant(File $phpcsFile, $stackPtr)
    {
        ...
        $validScopes = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_INTERFACE' => \true);
    ...
    protected function validDirectScope(File $phpcsFile, $stackPtr, $validScopes)
    {
        ...
        if (isset($validScopes[$tokens[$ptr]['type']]) === \true) {
            return $ptr;
        }
        return \false;
    }
    ...
    public function isUseOfGlobalConstant(File $phpcsFile, $stackPtr)
    {
        ...
        if ($next !== \false && ($tokens[$next]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS || $tokens[$next]['code'] === \T_DOUBLE_COLON)) {
            // Function call or declaration.
            return \false;
        }
        // Array of tokens which if found preceding the $stackPtr indicate that a T_STRING is not a global constant.
        $tokensToIgnore = array('T_NAMESPACE' => \true, 'T_USE' => \true, 'T_CLASS' => \true, 'T_TRAIT' => \true, 'T_INTERFACE' => \true, 'T_EXTENDS' => \true, 'T_IMPLEMENTS' => \true, 'T_NEW' => \true, 'T_FUNCTION' => \true, 'T_DOUBLE_COLON' => \true, 'T_OBJECT_OPERATOR' => \true, 'T_INSTANCEOF' => \true, 'T_INSTEADOF' => \true, 'T_GOTO' => \true, 'T_AS' => \true, 'T_PUBLIC' => \true, 'T_PROTECTED' => \true, 'T_PRIVATE' => \true);
        $prev = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if ($prev !== \false && (isset($tokensToIgnore[$tokens[$prev]['type']]) === \true || $tokens[$prev]['code'] === \T_STRING && ($isLowPHPCS === \true && $tokens[$prev]['content'] === 'trait' || $isLowPHP === \true && $tokens[$prev]['content'] === 'insteadof'))) {
            // Not the use of a constant.
            return \false;
        }
        if ($prev !== \false && $tokens[$prev]['code'] === \T_NS_SEPARATOR && $tokens[$prev - 1]['code'] === \T_STRING) {
            // Namespaced constant of the same name.
            return \false;
        }
        if ($prev !== \false && $tokens[$prev]['code'] === \T_CONST && $this->isClassConstant($phpcsFile, $prev) === \true) {
        ...
        if ($firstOnLine !== \false && $tokens[$firstOnLine]['code'] === \T_USE) {
            ...
            if ($nextOnLine !== \false) {
                if ($tokens[$nextOnLine]['code'] === \T_STRING && $tokens[$nextOnLine]['content'] === 'const' || $tokens[$nextOnLine]['code'] === \T_CONST) {
    ...
    protected function isNumber(File $phpcsFile, $start, $end, $allowFloats = \false)
    {
        ...
        $validTokens = array();
        $validTokens[\T_LNUMBER] = \true;
        $validTokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_TRUE] = \true;
        // Evaluates to int 1.
        $validTokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_FALSE] = \true;
        // Evaluates to int 0.
        $validTokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NULL] = \true;
        // Evaluates to int 0.
        if ($allowFloats === \true) {
            $validTokens[\T_DNUMBER] = \true;
        }
        ...
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $start, $searchEnd, \true);
        while ($nextNonEmpty !== \false && ($tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS || $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS)) {
            if ($tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS) {
                $negativeNumber = $negativeNumber === \false ? \true : \false;
            }
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, $searchEnd, \true);
        }
        if ($nextNonEmpty === \false || isset($maybeValidTokens[$tokens[$nextNonEmpty]['code']]) === \false) {
            return \false;
        }
        $content = \false;
        if ($tokens[$nextNonEmpty]['code'] === \T_LNUMBER || $tokens[$nextNonEmpty]['code'] === \T_DNUMBER) {
            $content = (float) $tokens[$nextNonEmpty]['content'];
        } elseif ($tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_TRUE) {
            $content = 1.0;
        } elseif ($tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_FALSE || $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NULL) {
            $content = 0.0;
        } elseif (isset($stringTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
            if ($tokens[$nextNonEmpty]['code'] === \T_START_HEREDOC || $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_START_NOWDOC) {
                // Skip past heredoc/nowdoc opener to the first content.
                $firstDocToken = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC), $nextNonEmpty + 1, $searchEnd);
                ...
                $nextNonEmpty = $phpcsFile->findNext(array(\T_END_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_END_NOWDOC), $nextNonEmpty + 1, $searchEnd);
            ...
            if ($intString === 1 && \trim($intMatch[0]) === '0' && \preg_match('`^\\s*(0x[A-Fa-f0-9]+)`', $stringContent, $hexNumberString) === 1 && $this->supportsBelow('5.6') === \true) {
    ...
    protected function isNumericCalculation(File $phpcsFile, $start, $end)
    {
        ...
        $skipTokens[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS;
        $skipTokens[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS;
        // Find the first arithmetic operator, but skip past +/- signs before numbers.
        $nextNonEmpty = $start - 1;
        do {
            $nextNonEmpty = $phpcsFile->findNext($skipTokens, $nextNonEmpty + 1, $end + 1, \true);
        ...
        while ($this->isNumber($phpcsFile, $subsetStart, $subsetEnd, \true) !== \false && isset($tokens[$arithmeticOperator + 1]) === \true) {
            // Recognize T_POW for PHPCS < 2.4.0 on low PHP versions.
            if (\defined('T_POW') === \false && $tokens[$arithmeticOperator]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MULTIPLY && $tokens[$arithmeticOperator + 1]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MULTIPLY && isset($tokens[$arithmeticOperator + 2]) === \true) {
            ...
            if ($arithmeticOperator === \false) {
                ...
                if ($this->isNumber($phpcsFile, $subsetStart, $end, \true) !== \false) {
                    return \true;
                }
                return \false;
            }
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN) {
            return \false;
        }
        ...
        if ($tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE) {
            return \true;
        }
        return \false;
    }
    ...
    public function isShortList(File $phpcsFile, $stackPtr)
    {
        ...
        // Is this one of the tokens this function handles ?
        if ($tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY && $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY) {
            return \false;
        }
        switch ($tokens[$stackPtr]['code']) {
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY:
                if (isset($tokens[$stackPtr]['bracket_closer']) === \true) {
                    $opener = $stackPtr;
                    $closer = $tokens[$stackPtr]['bracket_closer'];
                }
                break;
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY:
                if (isset($tokens[$stackPtr]['bracket_opener']) === \true) {
         ...
         */
        $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $opener - 1, null, \true, null, \true);
        if ($prevNonEmpty !== \false && $tokens[$prevNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET && isset($tokens[$prevNonEmpty]['bracket_opener']) === \true) {
            $maybeVariableVariable = $phpcsFile->findPrevious(Tokens::$emptyTokens, $tokens[$prevNonEmpty]['bracket_opener'] - 1, null, \true, null, \true);
            if ($tokens[$maybeVariableVariable]['code'] === \T_VARIABLE || $tokens[$maybeVariableVariable]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR) {
                return \false;
            }
        }
        $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $closer + 1, null, \true, null, \true);
        if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL) {
            return \true;
        }
        if ($prevNonEmpty !== \false && $tokens[$prevNonEmpty]['code'] === \T_AS && isset($tokens[$prevNonEmpty]['nested_parenthesis']) === \true) {
            $parentheses = \array_reverse($tokens[$prevNonEmpty]['nested_parenthesis'], \true);
            foreach ($parentheses as $open => $close) {
                if (isset($tokens[$open]['parenthesis_owner']) && $tokens[$tokens[$open]['parenthesis_owner']]['code'] === \T_FOREACH) {
                    return \true;
                }
            }
        }
        ...
        $parentOpener = $opener;
        do {
            $parentOpener = $phpcsFile->findPrevious(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET), $parentOpener - 1, null, \false, null, \true);
            if ($parentOpener === \false) {
                return \false;
            }
        } while (isset($tokens[$parentOpener]['bracket_closer']) === \true && $tokens[$parentOpener]['bracket_closer'] < $opener);
        if (isset($tokens[$parentOpener]['bracket_closer']) === \true && $tokens[$parentOpener]['bracket_closer'] > $closer) {
            // Work around tokenizer issue in PHPCS 2.0 - 2.7.
            $phpcsVersion = PHPCSHelper::getVersion();
            if (\version_compare($phpcsVersion, '2.0', '>') === \true && \version_compare($phpcsVersion, '2.8', '<') === \true && $tokens[$parentOpener]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET) {
                $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $tokens[$parentOpener]['bracket_closer'] + 1, null, \true, null, \true);
                if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL) {
                    return \true;
                }
                return \false;
    ...
    public function isVariable(File $phpcsFile, $start, $end, $targetNestingLevel)
    {
        ...
        if (isset($tokenBlackList, $bracketTokens) === \false) {
            $tokenBlackList = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_STRING_CONCAT => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_STRING_CONCAT);
             ...
             */
            $bracketTokens = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET);
        }
        $tokens = $phpcsFile->getTokens();
        ...
        if (isset($tokens[$hasVariable]['nested_parenthesis']) && \count($tokens[$hasVariable]['nested_parenthesis']) !== $targetNestingLevel) {
            return \false;
        }
    ...
    {
        $tokens = $phpcsFile->getTokens();
        if (isset($tokens[$stackPtr]) === \false || $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS && $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS) {
            return \false;
        }
        $next = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        if ($next === \false) {
        ...
        if (isset(Tokens::$operators[$tokens[$prev]['code']]) === \true) {
            // Just trying to operate on a positive/negative value; eg. ($var * -1).
            return \true;
        }
        if (isset(Tokens::$comparisonTokens[$tokens[$prev]['code']]) === \true) {
        ...
        if (isset(Tokens::$assignmentTokens[$tokens[$prev]['code']]) === \true) {
            // Just trying to assign a positive/negative value; eg. ($var = -1).
            return \true;
        }
        if (isset(Tokens::$castTokens[$tokens[$prev]['code']]) === \true) {
            // Just casting a positive/negative value; eg. (string) -$var.
            return \true;
        }
        // Other indicators that a plus/minus sign is a unary operator.
        $invalidTokens = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE => \true, \T_CASE => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_STRING_CONCAT => \true);
        if (isset($invalidTokens[$tokens[$prev]['code']]) === \true) {
     ...
     */
    public function getCompleteTextString(File $phpcsFile, $stackPtr, $stripQuotes = \true)
    {
        $tokens = $phpcsFile->getTokens();
        // Must be the start of a text string token.
        if ($tokens[$stackPtr]['code'] !== \T_START_HEREDOC && $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_START_NOWDOC && $tokens[$stackPtr]['code'] !== \T_CONSTANT_ENCAPSED_STRING && $tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING) {
        ...
        if ($tokens[$stackPtr]['code'] === \T_CONSTANT_ENCAPSED_STRING || $tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING) {
        ...
        switch ($tokens[$stackPtr]['code']) {
            case \T_START_HEREDOC:
                $stripQuotes = \false;
                $targetType = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC;
                $current = $stackPtr + 1;
                break;
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_START_NOWDOC:
                $stripQuotes = \false;
                $targetType = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC;
                $current = $stackPtr + 1;
                break;
            default:
