...
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\FunctionUse\RequiredToOptionalFunctionParametersSniff;
...
class OptionalToRequiredFunctionParametersSniff extends RequiredToOptionalFunctionParametersSniff
{
    ...
    protected $functionParameters = array(
        ...
        'crypt' => array(1 => array('name' => 'salt', '5.6' => 'recommended')),
        'parse_str' => array(1 => array('name' => 'result', '7.2' => \false)),
    );
    /**
    ...
    {
        return $errorInfo['optionalDeprecated'] !== '' || $errorInfo['optionalRemoved'] !== '' || $errorInfo['optionalRecommended'] !== '';
    }
    /**
    ...
    {
        $errorInfo = array('paramName' => '', 'optionalRecommended' => '', 'optionalDeprecated' => '', 'optionalRemoved' => '', 'error' => \false);
        $versionArray = $this->getVersionArray($itemArray);
        if (empty($versionArray) === \false) {
            foreach ($versionArray as $version => $required) {
                if ($this->supportsAbove($version) === \true) {
                    if ($required === \true && $errorInfo['optionalRemoved'] === '') {
    ...
    public function addError(File $phpcsFile, $stackPtr, array $itemInfo, array $errorInfo)
    {
        ...
        $data = array($errorInfo['paramName'], $itemInfo['name']);
        if ($errorInfo['optionalRecommended'] !== '') {
            $error .= 'since PHP %s ';
