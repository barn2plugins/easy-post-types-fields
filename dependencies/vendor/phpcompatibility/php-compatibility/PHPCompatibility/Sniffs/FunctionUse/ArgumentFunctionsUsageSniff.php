...
class ArgumentFunctionsUsageSniff extends Sniff
{
     ...
     * @var array
     */
    protected $targetFunctions = array('func_get_args' => \true, 'func_get_arg' => \true, 'func_num_args' => \true);
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $ignore = array(\T_DOUBLE_COLON => \true, \T_OBJECT_OPERATOR => \true, \T_FUNCTION => \true, \T_NEW => \true);
        $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset($ignore[$tokens[$prevNonEmpty]['code']]) === \true) {
         ...
         */
        if ($phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE)) === \false) {
            $isError = \false;
        ...
        if (isset($tokens[$closer]['parenthesis_owner']) && $tokens[$tokens[$closer]['parenthesis_owner']]['type'] === 'T_CLOSURE') {
            $throwError = \true;
        } else {
        ...
        $phpcsFile->addError('%s() could not be used in parameter lists prior to PHP 5.3.', $stackPtr, 'InParameterList', $data);
    }
}
