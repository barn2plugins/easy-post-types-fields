...
class ArgumentFunctionsReportCurrentValueSniff extends Sniff
{
     ...
     * @var array
     */
    protected $changedFunctions = array('func_get_arg' => \true, 'func_get_args' => \true, 'debug_backtrace' => \true, 'debug_print_backtrace' => \true);
    /**
     ...
     * @var array
     */
    private $skipPastNested = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_INTERFACE' => \true, 'T_TRAIT' => \true, 'T_FUNCTION' => \true, 'T_CLOSURE' => \true);
    /**
     ...
     * @var array
     */
    private $noneFunctionCallIndicators = array(\T_DOUBLE_COLON => \true, \T_OBJECT_OPERATOR => \true);
    /**
     ...
     * @var array
     */
    private $plusPlusMinusMinus = array(\T_DEC => \true, \T_INC => \true);
    /**
     ...
     * @var array
     */
    private $ignoreForStartOfStatement = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, \T_DOUBLE_ARROW, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS);
    /**
    ...
    {
        return array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $scopeOpener + 1; $i < $scopeCloser; $i++) {
            ...
            if ($next === \false || $tokens[$next]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
            ...
            if ($foundFunctionName !== 'func_get_args') {
                ...
                if ($paramOne !== \false) {
                    switch ($foundFunctionName) {
                        ...
                        case 'debug_print_backtrace':
                            $hasIgnoreArgs = $phpcsFile->findNext(\T_STRING, $paramOne['start'], $paramOne['end'] + 1, \false, 'DEBUG_BACKTRACE_IGNORE_ARGS');
                 ...
                 *  Should be exceptionally rare and can - if needs be - be addressed at a later stage.}
                 */
                if ($prev !== \false && $tokens[$prev]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
                    $maybeFunctionCall = $phpcsFile->findPrevious(Tokens::$emptyTokens, $prev - 1, null, \true);
                    if ($maybeFunctionCall !== \false && $tokens[$maybeFunctionCall]['code'] === \T_STRING && ($tokens[$maybeFunctionCall]['content'] === 'array_slice' || $tokens[$maybeFunctionCall]['content'] === 'array_splice')) {
                        $parentFuncParamTwo = $this->getFunctionCallParameter($phpcsFile, $maybeFunctionCall, 2);
                        $number = $phpcsFile->findNext(\T_LNUMBER, $parentFuncParamTwo['start'], $parentFuncParamTwo['end'] + 1);
            ...
            if ($foundFunctionName === 'debug_backtrace' && isset($tokens[$next]['parenthesis_closer'])) {
                $afterParenthesis = $phpcsFile->findNext(Tokens::$emptyTokens, $tokens[$next]['parenthesis_closer'] + 1, null, \true);
                if ($tokens[$afterParenthesis]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterParenthesis]['bracket_closer'])) {
                    $afterStackFrame = $phpcsFile->findNext(Tokens::$emptyTokens, $tokens[$afterParenthesis]['bracket_closer'] + 1, null, \true);
                    if ($tokens[$afterStackFrame]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterStackFrame]['bracket_closer'])) {
                        $arrayIndex = $phpcsFile->findNext(\T_CONSTANT_ENCAPSED_STRING, $afterStackFrame + 1, $tokens[$afterStackFrame]['bracket_closer']);
            ...
            for ($j = $scopeOpener + 1; $j < $startOfStatement; $j++) {
                if (isset($this->skipPastNested[$tokens[$j]['type']]) && isset($tokens[$j]['scope_closer'])) {
                    // Skip past nested structures.
                ...
                if ($foundFunctionName === 'func_get_arg' && isset($argNumber)) {
                    if (isset($paramNames[$argNumber]) && $tokens[$j]['content'] !== $paramNames[$argNumber]) {
                        // Different param than the one requested by func_get_arg().
                        continue;
                    }
                } elseif ($foundFunctionName === 'func_get_args' && isset($paramNamesSubset)) {
                    if (\in_array($tokens[$j]['content'], $paramNamesSubset, \true) === \false) {
                        // Different param than the ones requested by func_get_args().
                        continue;
                    }
                } elseif (\in_array($tokens[$j]['content'], $paramNames, \true) === \false) {
                ...
                $beforeVar = $phpcsFile->findPrevious(Tokens::$emptyTokens, $j - 1, null, \true);
                ...
                $afterVar = $phpcsFile->findNext(Tokens::$emptyTokens, $j + 1, null, \true);
                if ($afterVar === \false) {
                ...
                if ($tokens[$afterVar]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET && isset($tokens[$afterVar]['bracket_closer'])) {
                    // Skip past array access on the variable.
                    while (($afterVar = $phpcsFile->findNext(Tokens::$emptyTokens, $tokens[$afterVar]['bracket_closer'] + 1, null, \true)) !== \false) {
                        if ($tokens[$afterVar]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET || isset($tokens[$afterVar]['bracket_closer']) === \false) {
                            break;
                        }
                    }
                }
                if ($afterVar !== \false && isset(Tokens::$assignmentTokens[$tokens[$afterVar]['code']])) {
            ...
            $data = array($foundFunctionName, $tokens[$variableToken]['content'], $tokens[$variableToken]['line']);
            if ($scanResult === 'error') {
                $data[] = 'changed';
