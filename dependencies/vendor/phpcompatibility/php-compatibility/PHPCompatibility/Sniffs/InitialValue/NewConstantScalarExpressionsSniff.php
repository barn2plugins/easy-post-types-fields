 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\InitialValue;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniff;
...
class NewConstantScalarExpressionsSniff extends Sniff
{
     ...
     * @var array
     */
    protected $errorPhrases = array('const' => 'when defining constants using the const keyword', 'property' => 'in property declarations', 'staticvar' => 'in static variable declarations', 'default' => 'in default function arguments');
    /**
    ...
    protected $safeOperands = array(
        ...
        \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_TRUE => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_TRUE,
        \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_FALSE => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_FALSE,
        \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NULL => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NULL,
        \T_LINE => \T_LINE,
        \T_FILE => \T_FILE,
        \T_DIR => \T_DIR,
        \T_FUNC_C => \T_FUNC_C,
    ...
    public function register()
    {
        ...
        $this->setProperties();
        return array(\T_CONST, \T_VARIABLE, \T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE, \T_STATIC);
    }
    /**
    ...
    {
        return $this->supportsBelow('5.5') !== \true;
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        switch ($tokens[$stackPtr]['type']) {
            ...
            case 'T_CLOSURE':
                ...
                if (isset($funcToken['parenthesis_owner'], $funcToken['parenthesis_opener'], $funcToken['parenthesis_closer']) === \false || $funcToken['parenthesis_owner'] !== $stackPtr || isset($tokens[$funcToken['parenthesis_opener']], $tokens[$funcToken['parenthesis_closer']]) === \false) {
                ...
                foreach ($params as $param) {
                    ...
                    $end = $param['token'];
                    while (($end = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS), $end + 1, $closer + 1)) !== \false) {
                        ...
                        if ($tokens[$end]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS && $end !== $closer) {
                            continue;
                        }
            ...
            case 'T_CONST':
                ...
                if ($tokens[$stackPtr]['code'] === \T_STATIC) {
                    $next = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
                    if ($next === \false || $tokens[$next]['code'] !== \T_VARIABLE) {
                        // Late static binding.
                        return;
                    }
                    if ($this->isClassProperty($phpcsFile, $next) === \true) {
                ...
                $endOfStatement = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_CLOSE_TAG), $stackPtr + 1);
                ...
                $start = $stackPtr;
                $end = $stackPtr;
                while (($end = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \T_CLOSE_TAG), $end + 1, $endOfStatement + 1)) !== \false) {
                    $maybeSkipTo = $this->isRealEndOfDeclaration($tokens, $end, $targetNestingLevel);
                    if ($maybeSkipTo !== \true) {
                        $end = $maybeSkipTo;
                        continue;
                    }
                    $start = $phpcsFile->findNext(Tokens::$emptyTokens, $start + 1, $end, \true);
                    if ($start === \false || $tokens[$stackPtr]['code'] === \T_CONST && $tokens[$start]['code'] !== \T_STRING || $tokens[$stackPtr]['code'] !== \T_CONST && $tokens[$start]['code'] !== \T_VARIABLE) {
    ...
    {
        $tokens = $phpcsFile->getTokens();
        $next = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, $end, \true);
        if ($next === \false || $tokens[$next]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL) {
            // No value assigned.
            return \true;
        }
    ...
    {
        $nextNonSimple = $phpcsFile->findNext($this->safeOperands, $start, $end + 1, \true);
        ...
        switch ($tokens[$nextNonSimple]['code']) {
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS:
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS:
                if ($this->isNumber($phpcsFile, $start, $end, \true) !== \false) {
                    // Int or float with sign.
                    return \true;
                }
                return \false;
            case \T_NAMESPACE:
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT:
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF:
            case \T_DOUBLE_COLON:
                $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonSimple + 1, $end + 1, \true);
                if ($tokens[$nextNonSimple]['code'] === \T_NAMESPACE) {
                ...
                } elseif ($tokens[$nextNonSimple]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT || $tokens[$nextNonSimple]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF) {
                    ...
                    $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $nextNonSimple - 1, null, \true);
            ...
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY:
                ...
                if (empty($arrayItems) === \false) {
                    foreach ($arrayItems as $item) {
                        ...
                        $doubleArrow = \false;
                        $maybeDoubleArrow = $phpcsFile->findNext(array(\T_DOUBLE_ARROW, \T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY), $item['start'], $item['end'] + 1);
                ...
                if ($tokens[$nextNonSimple]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY && isset($tokens[$nextNonSimple]['bracket_closer']) === \true) {
                    $closer = $tokens[$nextNonSimple]['bracket_closer'];
                } else {
                    $maybeOpener = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonSimple + 1, $end + 1, \true);
                    if ($tokens[$maybeOpener]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
                        $opener = $maybeOpener;
                        if (isset($tokens[$opener]['parenthesis_closer']) === \true) {
    ...
    protected function throwError(File $phpcsFile, $stackPtr, $type, $content = '')
    {
        ...
        $phrase = '';
        $errorCode = 'Found';
        if (isset($this->errorPhrases[$type]) === \true) {
    ...
    private function isRealEndOfDeclaration($tokens, $endPtr, $targetLevel)
    {
        ...
        if ($tokens[$endPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY && (isset($tokens[$endPtr]['bracket_opener']) && $tokens[$endPtr]['bracket_opener'] === $endPtr) && isset($tokens[$endPtr]['bracket_closer'])) {
            // Skip forward to the end of the short array definition.
            return $tokens[$endPtr]['bracket_closer'];
        }
        // Skip past comma's at a lower nesting level.
        if ($tokens[$endPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA) {
            // Check if a comma is at the nesting level we're targetting.
            $nestingLevel = 0;
            if (isset($tokens[$endPtr]['nested_parenthesis']) === \true) {
