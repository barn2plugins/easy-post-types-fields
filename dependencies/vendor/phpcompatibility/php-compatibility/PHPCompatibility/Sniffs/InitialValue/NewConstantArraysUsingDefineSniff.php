...
class NewConstantArraysUsingDefineSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $array = $phpcsFile->findNext(array(\T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY), $secondParam['start'], $secondParam['end'] + 1);
        if ($array !== \false) {
            if (isset($tokens[$array]['nested_parenthesis']) === \false && $targetNestingLevel === 0 || \count($tokens[$array]['nested_parenthesis']) === $targetNestingLevel) {
                $phpcsFile->addError('Constant arrays using define are not allowed in PHP 5.6 or earlier', $array, 'Found');
            }
        }
    }
}
