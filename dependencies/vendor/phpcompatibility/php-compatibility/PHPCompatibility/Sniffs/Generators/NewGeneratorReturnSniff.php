 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Generators;
...
class NewGeneratorReturnSniff extends Sniff
{
     ...
     * @var array
     */
    private $validConditions = array(\T_FUNCTION => \T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
    /**
    ...
    public function register()
    {
         ...
         */
        if (\version_compare(\PHP_VERSION_ID, '50500', '<') === \true && \version_compare(PHPCSHelper::getVersion(), '3.1.0', '<') === \true) {
            $targets[] = \T_STRING;
        }
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \T_STRING && $tokens[$stackPtr]['content'] !== 'yield') {
            return;
        }
        ...
        foreach ($conditions as $ptr => $type) {
            if (isset($this->validConditions[$type]) === \true) {
                $function = $ptr;
                break;
            }
        }
        ...
        $targets = array(\T_RETURN, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE, \T_FUNCTION, \T_CLASS);
        ...
        while (($current = $phpcsFile->findNext($targets, $current + 1, $tokens[$function]['scope_closer'])) !== \false) {
            if ($tokens[$current]['code'] === \T_RETURN) {
                $phpcsFile->addError('Returning a final expression from a generator was not supported in PHP 5.6 or earlier', $current, 'ReturnFound');
