...
class RemovedAlternativePHPTagsSniff extends Sniff
{
    ...
    {
        if (\version_compare(\PHP_VERSION_ID, '70000', '<') === \true) {
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($openTag['code'] === \T_OPEN_TAG || $openTag['code'] === \T_OPEN_TAG_WITH_ECHO) {
            if ($content === '<%' || $content === '<%=') {
                $data = array('ASP', $content);
                $errorCode = 'ASPOpenTagFound';
            } elseif (\strpos($content, '<script ') !== \false) {
                $data = array('Script', $content);
                $errorCode = 'ScriptOpenTagFound';
            } else {
                return;
            }
        } elseif ($openTag['code'] === \T_INLINE_HTML && \preg_match('`((?:<s)?cript (?:[^>]+)?language=[\'"]?php[\'"]?(?:[^>]+)?>)`i', $content, $match) === 1) {
            $found = $match[1];
            $data = array('Script', $found);
            $errorCode = 'ScriptOpenTagFound';
        }
        if (isset($errorCode, $data)) {
            $phpcsFile->addError('%s style opening tags have been removed in PHP 7.0. Found "%s"', $stackPtr, $errorCode, $data);
            return;
        }
