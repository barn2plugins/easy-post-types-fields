 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Miscellaneous;
...
class ValidIntegersSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->couldBeBinaryInteger($tokens, $stackPtr) === \true) {
            ...
            if ($this->isInvalidBinaryInteger($tokens, $stackPtr) === \true) {
        ...
        $data = array($token['content']);
        if ($this->isInvalidOctalInteger($tokens, $stackPtr) === \true) {
            $this->addMessage($phpcsFile, 'Invalid octal integer detected. Prior to PHP 7 this would lead to a truncated number. From PHP 7 onwards this causes a parse error. Found: %s', $stackPtr, $isError, 'InvalidOctalIntegerFound', $data);
            return;
        }
        if ($this->isHexidecimalNumericString($tokens, $stackPtr) === \true) {
            $this->addMessage($phpcsFile, 'The behaviour of hexadecimal numeric strings was inconsistent prior to PHP 7 and support has been removed in PHP 7. Found: %s', $stackPtr, $isError, 'HexNumericStringFound', $data);
            return;
        }
    }
    /**
