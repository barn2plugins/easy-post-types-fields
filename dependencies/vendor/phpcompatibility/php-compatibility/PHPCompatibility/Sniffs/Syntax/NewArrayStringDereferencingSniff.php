...
class NewArrayStringDereferencingSniff extends Sniff
{
    ...
    {
        return array(\T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \T_CONSTANT_ENCAPSED_STRING);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($dereferencing['braces'] as $openBrace => $closeBrace) {
            if ($supports54 === \true && ($tokens[$openBrace]['type'] === 'T_OPEN_SQUARE_BRACKET' || $tokens[$openBrace]['type'] === 'T_OPEN_SHORT_ARRAY')) {
                $phpcsFile->addError('Direct array dereferencing of %s is not present in PHP version 5.4 or earlier', $openBrace, 'Found', array($dereferencing['type']));
                continue;
            }
            // PHP 7.0 Array/string dereferencing using curly braces.
            if ($tokens[$openBrace]['type'] === 'T_OPEN_CURLY_BRACKET') {
                $phpcsFile->addError('Direct array dereferencing of %s using curly braces is not present in PHP version 5.6 or earlier', $openBrace, 'FoundUsingCurlies', array($dereferencing['type']));
            }
        }
    }
    /**
    ...
    public function isArrayStringDereferencing(File $phpcsFile, $stackPtr)
    {
        ...
        $braces = array();
        do {
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $end + 1, null, \true, null, \true);
            if ($nextNonEmpty === \false) {
                break;
            }
            if ($tokens[$nextNonEmpty]['type'] === 'T_OPEN_SQUARE_BRACKET' || $tokens[$nextNonEmpty]['type'] === 'T_OPEN_CURLY_BRACKET' || $tokens[$nextNonEmpty]['type'] === 'T_OPEN_SHORT_ARRAY') {
        ...
        return array('type' => $type, 'braces' => $braces);
    }
}
