 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Syntax;
...
class ForbiddenCallTimePassByReferenceSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $findTokens[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_BITWISE_AND;
        $prevNonEmpty = $phpcsFile->findPrevious($findTokens, $stackPtr - 1, null, \true);
        if ($prevNonEmpty !== \false && \in_array($tokens[$prevNonEmpty]['type'], array('T_FUNCTION', 'T_CLASS', 'T_INTERFACE', 'T_TRAIT'), \true)) {
            return;
        }
        ...
        if ($openBracket === \false || $tokens[$openBracket]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS || isset($tokens[$openBracket]['parenthesis_closer']) === \false) {
            return;
        }
        ...
        foreach ($parameters as $parameter) {
            if ($this->isCallTimePassByReferenceParam($phpcsFile, $parameter, $nestedParenthesisCount) === \true) {
    ...
    protected function isCallTimePassByReferenceParam(File $phpcsFile, $parameter, $nestingLevel)
    {
        ...
        do {
            $nextVariable = $phpcsFile->findNext(array(\T_VARIABLE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE), $nextVariable + 1, $searchEndToken);
            if ($nextVariable === \false) {
                return \false;
            }
            // Ignore anything within short array definition brackets.
            if ($tokens[$nextVariable]['type'] === 'T_OPEN_SHORT_ARRAY' && (isset($tokens[$nextVariable]['bracket_opener']) && $tokens[$nextVariable]['bracket_opener'] === $nextVariable) && isset($tokens[$nextVariable]['bracket_closer'])) {
                // Skip forward to the end of the short array definition.
                $nextVariable = $tokens[$nextVariable]['bracket_closer'];
                continue;
            }
            // Skip past closures passed as function parameters.
            if ($tokens[$nextVariable]['type'] === 'T_CLOSURE' && (isset($tokens[$nextVariable]['scope_condition']) && $tokens[$nextVariable]['scope_condition'] === $nextVariable) && isset($tokens[$nextVariable]['scope_closer'])) {
            ...
            if (isset($tokens[$nextVariable]['nested_parenthesis']) === \false || \count($tokens[$nextVariable]['nested_parenthesis']) !== $nestingLevel) {
                continue;
            }
            // Checking this: $value = my_function(...[*]$arg...).
            $tokenBefore = $phpcsFile->findPrevious(Tokens::$emptyTokens, $nextVariable - 1, $searchStartToken, \true);
            if ($tokenBefore === \false || $tokens[$tokenBefore]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_BITWISE_AND) {
            ...
            // Checking this: $value = my_function(...[*]&$arg...).
            $tokenBefore = $phpcsFile->findPrevious(Tokens::$emptyTokens, $tokenBefore - 1, $searchStartToken, \true);
