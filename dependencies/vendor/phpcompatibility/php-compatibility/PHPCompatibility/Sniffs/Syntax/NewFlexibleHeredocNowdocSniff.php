...
class NewFlexibleHeredocNowdocSniff extends Sniff
{
    ...
    {
        $targets = array(\T_END_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_END_NOWDOC);
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
         ...
         */
        if ($this->supportsBelow('7.2') === \true && \version_compare(PHPCSHelper::getVersion(), '2.6.0', '>=')) {
            $this->detectIndentedNonStandAloneClosingMarker($phpcsFile, $stackPtr);
        }
        $tokens = $phpcsFile->getTokens();
        if ($this->supportsAbove('7.3') === \true && $tokens[$stackPtr]['code'] !== \T_STRING) {
    ...
    protected function detectIndentedNonStandAloneClosingMarker(File $phpcsFile, $stackPtr)
    {
        ...
        if (\version_compare(\PHP_VERSION_ID, '70299', '>') === \true) {
            /*
             ...
             */
            $nextNonWhitespace = $phpcsFile->findNext(array(\T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON), $stackPtr + 1, null, \true);
    ...
    protected function detectClosingMarkerInBody(File $phpcsFile, $stackPtr)
    {
        ...
        if (\version_compare(\PHP_VERSION_ID, '70299', '>') === \true) {
            $nextNonWhitespace = $phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, null, \true, null, \true);
            if ($nextNonWhitespace === \false || $tokens[$nextNonWhitespace]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON || ($tokens[$nextNonWhitespace]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA || $tokens[$nextNonWhitespace]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_STRING_CONCAT) && $tokens[$nextNonWhitespace]['line'] !== $tokens[$stackPtr]['line']) {
                // This is most likely a correctly identified closing marker.
                return;
            }
            // The real closing tag has to be before the next heredoc/nowdoc.
            $nextHereNowDoc = $phpcsFile->findNext(array(\T_START_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_START_NOWDOC), $stackPtr + 1);
            ...
            while (($realClosingMarker = $phpcsFile->findNext(\T_STRING, $realClosingMarker + 1, $nextHereNowDoc, \false, $identifier)) !== \false) {
                $prevNonWhitespace = $phpcsFile->findPrevious(\T_WHITESPACE, $realClosingMarker - 1, null, \true);
                if ($prevNonWhitespace === \false || $tokens[$prevNonWhitespace]['line'] === $tokens[$realClosingMarker]['line']) {
            ...
            if (isset($tokens[$stackPtr]['scope_closer'], $tokens[$stackPtr]['scope_opener']) === \true && $tokens[$stackPtr]['scope_closer'] === $stackPtr) {
                $opener = $tokens[$stackPtr]['scope_opener'];
            } else {
                // PHPCS < 3.0.2 did not add scope_* values for Nowdocs.
                $opener = $phpcsFile->findPrevious(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_START_NOWDOC, $stackPtr - 1);
                if ($opener === \false) {
                    return;
                }
            }
