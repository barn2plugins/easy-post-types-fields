...
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Syntax\NewArrayStringDereferencingSniff;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Syntax\NewClassMemberAccessSniff;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Syntax\NewFunctionArrayDereferencingSniff;
...
class RemovedCurlyBraceArrayAccessSniff extends Sniff
{
    ...
    {
        $targets = array(array(\T_VARIABLE, \T_STRING));
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($braces as $open => $close) {
            ...
            if ($tokens[$open]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET) {
                continue;
            }
            // Make sure there is something between the braces, otherwise it's still not curly brace array access.
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $open + 1, $close, \true);
            ...
            $fix = $phpcsFile->addFixableWarning('Curly brace syntax for accessing array elements and string offsets has been deprecated in PHP 7.4. Found: %s', $open, 'Found', array($snippet));
            if ($fix === \true) {
    ...
    protected function isVariableArrayAccess(File $phpcsFile, $stackPtr)
    {
        ...
        $braces = array();
        do {
            $current = $phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, null, \true);
            if ($current === \false) {
                break;
            }
            // Skip over square bracket array access. Bracket styles can be mixed.
            if ($tokens[$current]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET && isset($tokens[$current]['bracket_closer']) === \true && $current === $tokens[$current]['bracket_opener']) {
                $current = $tokens[$current]['bracket_closer'];
                continue;
            }
            // Handle property access.
            if ($tokens[$current]['code'] === \T_OBJECT_OPERATOR) {
                $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, null, \true);
            ...
            if ($tokens[$current]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET) {
    ...
    protected function isConstantArrayAccess(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->isUseOfGlobalConstant($phpcsFile, $stackPtr) === \false && $tokens[$prevNonEmpty]['code'] !== \T_DOUBLE_COLON) {
            return array();
        }
        ...
        if ($tokens[$nextNonEmpty]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET || isset($tokens[$nextNonEmpty]['bracket_closer']) === \false) {
