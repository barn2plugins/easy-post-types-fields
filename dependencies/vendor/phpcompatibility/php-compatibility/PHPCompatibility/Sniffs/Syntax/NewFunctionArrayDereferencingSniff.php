...
class NewFunctionArrayDereferencingSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($dereferencing as $openBrace => $closeBrace) {
            if ($supports53 === \true && $tokens[$openBrace]['type'] === 'T_OPEN_SQUARE_BRACKET') {
                $phpcsFile->addError('Function array dereferencing is not present in PHP version 5.3 or earlier', $openBrace, 'Found');
                continue;
            }
            // PHP 7.0 function array dereferencing using curly braces.
            if ($tokens[$openBrace]['type'] === 'T_OPEN_CURLY_BRACKET') {
                $phpcsFile->addError('Function array dereferencing using curly braces is not present in PHP version 5.6 or earlier', $openBrace, 'FoundUsingCurlies');
            }
        }
    }
    /**
    ...
    public function isFunctionArrayDereferencing(File $phpcsFile, $stackPtr)
    {
        ...
        if ($openParenthesis === \false || $tokens[$openParenthesis]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
            return array();
        }
        ...
        if ($prevToken !== \false && \in_array($tokens[$prevToken]['code'], array(\T_DOUBLE_COLON, \T_OBJECT_OPERATOR), \true) === \false) {
            if ($tokens[$prevToken]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_BITWISE_AND) {
                // This may be a function declared by reference.
                $prevToken = $phpcsFile->findPrevious(Tokens::$emptyTokens, $prevToken - 1, null, \true);
            }
            $ignore = array(\T_FUNCTION => \true, \T_CONST => \true, \T_USE => \true, \T_NEW => \true, \T_CLASS => \true, \T_INTERFACE => \true);
        ...
        $braces = array();
        do {
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, null, \true, null, \true);
            if ($nextNonEmpty === \false) {
                break;
            }
            if ($tokens[$nextNonEmpty]['type'] === 'T_OPEN_SQUARE_BRACKET' || $tokens[$nextNonEmpty]['type'] === 'T_OPEN_CURLY_BRACKET') {
