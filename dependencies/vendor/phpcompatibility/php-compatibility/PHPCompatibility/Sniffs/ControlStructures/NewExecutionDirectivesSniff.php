 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\ControlStructures;
...
class NewExecutionDirectivesSniff extends AbstractNewFeatureSniff
{
     ...
     */
    protected $newDirectives = array('ticks' => array('3.1' => \false, '4.0' => \true, 'valid_value_callback' => 'isNumeric'), 'encoding' => array(
        '5.2' => \false,
        '5.3' => '--enable-zend-multibyte',
        // Directive ignored unless.
        '5.4' => \true,
        'valid_value_callback' => 'validEncoding',
    ), 'strict_types' => array('5.6' => \false, '7.0' => \true, 'valid_values' => array(1)));
    /**
    ...
    public function register()
    {
        ...
        $this->ignoreTokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL;
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['parenthesis_opener'], $tokens[$stackPtr]['parenthesis_closer']) === \true) {
            ...
            $openParenthesis = $phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS, $stackPtr + 1, null, \false, null, \true);
        ...
        if (isset($this->newDirectives[$directiveContent]) === \false) {
            ...
            $data = array(\implode(', ', \array_keys($this->newDirectives)), $directiveContent);
            $phpcsFile->addError($error, $stackPtr, 'InvalidDirectiveFound', $data);
        } else {
            // Check for valid directive for version.
            $itemInfo = array('name' => $directiveContent);
            $this->handleFeature($phpcsFile, $stackPtr, $itemInfo);
            // Check for valid directive value.
            $valuePtr = $phpcsFile->findNext($this->ignoreTokens, $directivePtr + 1, $closeParenthesis, \true);
            if ($valuePtr === \false) {
                return;
            }
    ...
    {
        return array('valid_value_callback', 'valid_values');
    }
    /**
    ...
    public function getErrorInfo(array $itemArray, array $itemInfo)
    {
        ...
        if (empty($versionArray) === \false) {
            foreach ($versionArray as $version => $present) {
                if (\is_string($present) === \true && $this->supportsBelow($version) === \true) {
    ...
    public function addError(File $phpcsFile, $stackPtr, array $itemInfo, array $errorInfo)
    {
        if ($errorInfo['not_in_version'] !== '') {
            ...
            $data = array($itemInfo['name'], $errorInfo['conditional_version'], $errorInfo['condition']);
    ...
    protected function validEncoding($value)
    {
        ...
        return \in_array($value, $encodings, \true);
    }
}
