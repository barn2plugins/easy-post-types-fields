...
class NewForeachExpressionReferencingSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
         ...
         */
        $hasReference = $phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_BITWISE_AND, $asToken + 1, $closer);
        ...
        if ($this->isVariable($phpcsFile, $opener + 1, $asToken, $nestingLevel) === \true) {
            return;
        }
        // Non-variable detected before the `as` keyword.
        $phpcsFile->addError('Referencing $value is only possible if the iterated array is a variable in PHP 5.4 or earlier.', $hasReference, 'Found');
    }
}
