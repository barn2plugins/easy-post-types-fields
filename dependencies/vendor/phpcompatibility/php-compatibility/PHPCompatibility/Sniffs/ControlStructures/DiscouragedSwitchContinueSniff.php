...
class DiscouragedSwitchContinueSniff extends Sniff
{
     ...
     * @var array
     */
    protected $loopStructures = array(\T_FOR => \T_FOR, \T_FOREACH => \T_FOREACH, \T_WHILE => \T_WHILE, \T_DO => \T_DO, \T_SWITCH => \T_SWITCH);
    /**
     ...
     * @var array
     */
    protected $caseTokens = array(\T_CASE => \T_CASE, \T_DEFAULT => \T_DEFAULT);
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        do {
            ...
            do {
                ...
                $nextSemicolon = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_CLOSE_TAG), $continue + 1, $caseCloser);
                $codeString = '';
                ...
                $conditions = \array_reverse($tokens[$continue]['conditions'], \true);
                ...
                $targetCondition = \array_slice($conditions, $level - 1, 1, \true);
                if (empty($targetCondition)) {
                    continue;
                }
                $conditionToken = \key($targetCondition);
                if ($conditionToken === $stackPtr) {
                    $phpcsFile->addWarning("Targeting a 'switch' control structure with a 'continue' statement is strongly discouraged and will throw a warning as of PHP 7.3.", $continue, 'Found');
                }
