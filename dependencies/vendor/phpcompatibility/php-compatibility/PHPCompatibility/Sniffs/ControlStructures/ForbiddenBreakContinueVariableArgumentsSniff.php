...
class ForbiddenBreakContinueVariableArgumentsSniff extends Sniff
{
     ...
     * @var array
     */
    private $errorTypes = array('variableArgument' => 'a variable argument', 'zeroArgument' => '0 as an argument');
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $nextSemicolonToken = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_CLOSE_TAG), $stackPtr, null, \false);
        $errorType = '';
        for ($curToken = $stackPtr + 1; $curToken < $nextSemicolonToken; $curToken++) {
            if ($tokens[$curToken]['type'] === 'T_STRING') {
                ...
                $openBracket = $phpcsFile->findNext(Tokens::$emptyTokens, $curToken + 1, null, \true);
                if ($tokens[$openBracket]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
                    $errorType = 'variableArgument';
                    break;
                }
            } elseif (\in_array($tokens[$curToken]['type'], array('T_VARIABLE', 'T_FUNCTION', 'T_CLOSURE'), \true)) {
                $errorType = 'variableArgument';
                break;
