 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\ParameterValues;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\AbstractFunctionCallParameterSniff;
...
class RemovedImplodeFlexibleParamOrderSniff extends AbstractFunctionCallParameterSniff
{
     ...
     * @var array
     */
    protected $targetFunctions = array('implode' => \true, 'join' => \true);
    /**
     ...
     * @var array
     */
    private $constantStrings = array('DIRECTORY_SEPARATOR' => \true, 'PHP_EOL' => \true);
    /**
     ...
     * @var array
     */
    private $arrayFunctions = array('compact' => \true, 'explode' => \true, 'range' => \true);
    /**
     ...
     * @var array
     */
    private $arrayFunctionExceptions = array('array_key_exists' => \true, 'array_key_first' => \true, 'array_key_last' => \true, 'array_multisort' => \true, 'array_pop' => \true, 'array_product' => \true, 'array_push' => \true, 'array_search' => \true, 'array_shift' => \true, 'array_sum' => \true, 'array_unshift' => \true, 'array_walk_recursive' => \true, 'array_walk' => \true);
    /**
    ...
    public function processParameters(File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        $isOnlyText = \true;
        $firstNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $start, $end, \true);
        if ($firstNonEmpty === \false) {
            // Parse error. Shouldn't be possible.
            return;
        }
        if ($tokens[$firstNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS) {
            $start = $firstNonEmpty + 1;
            $end = $tokens[$firstNonEmpty]['parenthesis_closer'];
        }
        $hasTernary = $phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN, $start, $end);
        if ($hasTernary !== \false && isset($tokens[$start]['nested_parenthesis'], $tokens[$hasTernary]['nested_parenthesis']) && \count($tokens[$start]['nested_parenthesis']) === \count($tokens[$hasTernary]['nested_parenthesis'])) {
            $start = $hasTernary + 1;
        }
        for ($i = $start; $i < $end; $i++) {
            ...
            if ($hasTernary !== \false && $tokenCode === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE) {
                continue;
            }
            if (isset(Tokens::$stringTokens[$tokenCode]) === \false) {
                $isOnlyText = \false;
            }
            if ($tokenCode === \T_ARRAY || $tokenCode === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY || $tokenCode === \T_ARRAY_CAST) {
            ...
            if ($tokenCode === \T_STRING) {
                 ...
                 */
                $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, $end, \true);
                ...
                if (isset($this->arrayFunctions[$nameLc]) === \false && (\strpos($nameLc, 'array_') !== 0 || isset($this->arrayFunctionExceptions[$nameLc]) === \true)) {
                    continue;
                }
                // Now make sure it's the PHP native function being called.
                $prevNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $i - 1, $start, \true);
                if ($tokens[$prevNonEmpty]['code'] === \T_DOUBLE_COLON || $tokens[$prevNonEmpty]['code'] === \T_OBJECT_OPERATOR) {
                    // Method call, not a call to the PHP native function.
                    continue;
                }
                if ($tokens[$prevNonEmpty]['code'] === \T_NS_SEPARATOR && $tokens[$prevNonEmpty - 1]['code'] === \T_STRING) {
                    // Namespaced function.
                    continue;
                }
        ...
        for ($i = $start; $i < $end; $i++) {
            ...
            if ($tokenCode === \T_CONSTANT_ENCAPSED_STRING || $tokenCode === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING || $tokenCode === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC || $tokenCode === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC) {
                $this->throwNotice($phpcsFile, $stackPtr, $functionName);
                return;
            }
        }
