...
class NewIconvMbstringCharsetDefaultSniff extends AbstractFunctionCallParameterSniff
{
    ...
    public function processParameters(File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        if (isset($parameters[$this->targetFunctions[$functionLC]]) === \true) {
            return;
        }
        $paramName = '$encoding';
        ...
        $data = array($paramName, $functionName);
    ...
    public function processIconvMimeEncode(File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        if (isset($parameters[$this->targetFunctions[$functionLC]]) === \false) {
            $phpcsFile->addError($error, $stackPtr, 'PreferencesNotSet', array('$preferences[\'input/output-charset\']', '$preferences[\'input-charset\'] and $preferences[\'output-charset\'] indexes'));
            return;
        }
        $tokens = $phpcsFile->getTokens();
        $targetParam = $parameters[$this->targetFunctions[$functionLC]];
        $firstNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $targetParam['start'], $targetParam['end'] + 1, \true);
        if ($firstNonEmpty === \false) {
            // Parse error or live coding.
            return;
        }
        if ($tokens[$firstNonEmpty]['code'] === \T_ARRAY || $tokens[$firstNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY) {
            ...
            if ($hasInputCharset !== 1) {
                $phpcsFile->addError($error, $firstNonEmpty, 'InputPreferenceNotSet', array('$preferences[\'input-charset\']', '$preferences[\'input-charset\'] index'));
            }
            if ($hasOutputCharset !== 1) {
                $phpcsFile->addError($error, $firstNonEmpty, 'OutputPreferenceNotSet', array('$preferences[\'output-charset\']', '$preferences[\'output-charset\'] index'));
            }
            return;
        }
        // The $preferences parameter was passed, but it was a variable/constant/output of a function call.
        $phpcsFile->addWarning($error, $firstNonEmpty, 'Undetermined', array('$preferences[\'input/output-charset\']', '$preferences[\'input-charset\'] and $preferences[\'output-charset\'] indexes'));
    }
}
