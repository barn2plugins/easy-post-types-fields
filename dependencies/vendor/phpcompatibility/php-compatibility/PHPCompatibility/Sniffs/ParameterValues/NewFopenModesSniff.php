...
class NewFopenModesSniff extends AbstractFunctionCallParameterSniff
{
     ...
     * @var array
     */
    protected $targetFunctions = array('fopen' => \true);
    /**
    ...
    public function processParameters(File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        for ($i = $targetParam['start']; $i <= $targetParam['end']; $i++) {
            ...
            if (\strpos($tokens[$i]['content'], 'c+') !== \false && $this->supportsBelow('5.2.5')) {
                $errors['cplusFound'] = array('c+', '5.2.5', $targetParam['raw']);
            } elseif (\strpos($tokens[$i]['content'], 'c') !== \false && $this->supportsBelow('5.2.5')) {
                $errors['cFound'] = array('c', '5.2.5', $targetParam['raw']);
            }
            if (\strpos($tokens[$i]['content'], 'e') !== \false && $this->supportsBelow('7.0.15')) {
                $errors['eFound'] = array('e', '7.0.15', $targetParam['raw']);
            }
        }
        if (empty($errors) === \true) {
            return;
        ...
        foreach ($errors as $errorCode => $errorData) {
            $phpcsFile->addError('Passing "%s" as the $mode to fopen() is not supported in PHP %s or lower. Found %s', $targetParam['start'], $errorCode, $errorData);
        }
    }
}
