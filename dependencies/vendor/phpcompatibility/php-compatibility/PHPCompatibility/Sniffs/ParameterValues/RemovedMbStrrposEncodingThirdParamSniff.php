...
class RemovedMbStrrposEncodingThirdParamSniff extends AbstractFunctionCallParameterSniff
{
     ...
     * @var array
     */
    protected $targetFunctions = array('mb_strrpos' => \true);
    /**
     ...
     * @var array
     */
    private $textStringTokens = array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC);
    /**
     ...
     * @var array
     */
    private $numberTokens = array(\T_LNUMBER => \T_LNUMBER, \T_DNUMBER => \T_DNUMBER, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PLUS);
    /**
    ...
    public function processParameters(File $phpcsFile, $stackPtr, $functionName, $parameters)
    {
        ...
        if (isset($parameters[4]) === \true) {
            // Encoding set as fourth parameter.
            return;
        }
        $targetParam = $parameters[3];
        $targets = $this->numberTokens + Tokens::$emptyTokens;
        $nonNumber = $phpcsFile->findNext($targets, $targetParam['start'], $targetParam['end'] + 1, \true);
        if ($nonNumber === \false) {
            return;
        }
        if ($this->isNumericCalculation($phpcsFile, $targetParam['start'], $targetParam['end']) === \true) {
            return;
        }
        ...
        $phpcsFile->addWarning($error, $targetParam['start'], 'Deprecated');
    }
}
