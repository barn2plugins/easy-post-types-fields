...
class NewKeyedListSniff extends Sniff
{
     ...
     * @var array
     */
    protected $sniffTargets = array(\T_LIST => \T_LIST, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY);
    /**
     ...
     * @var array
     */
    protected $targetsInList = array(\T_DOUBLE_ARROW => \T_DOUBLE_ARROW);
    /**
    ...
    protected function examineList(File $phpcsFile, $opener, $closer)
    {
        ...
        while (($start = $this->hasTargetInList($phpcsFile, $start, $closer)) !== \false) {
            $phpcsFile->addError('Specifying keys in list constructs is not supported in PHP 7.0 or earlier.', $start, 'Found');
        }
    }
    /**
    ...
    protected function hasTargetInList(File $phpcsFile, $start, $closer)
    {
        ...
        for ($i = $start + 1; $i < $closer; $i++) {
            ...
            if (isset($this->targetsInList[$tokens[$i]['code']]) === \true) {
                return $i;
            }
            // Skip past nested list constructs.
            if ($tokens[$i]['code'] === \T_LIST) {
                $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
                if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS && isset($tokens[$nextNonEmpty]['parenthesis_closer']) === \true) {
                    $i = $tokens[$nextNonEmpty]['parenthesis_closer'];
                }
            } elseif ($tokens[$i]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY && isset($tokens[$i]['bracket_closer'])) {
