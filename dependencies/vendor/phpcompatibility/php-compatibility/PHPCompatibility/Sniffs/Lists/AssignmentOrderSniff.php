 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Lists;
...
class AssignmentOrderSniff extends Sniff
{
    ...
    {
        return array(\T_LIST, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY && $this->isShortList($phpcsFile, $stackPtr) === \false) {
            // Short array, not short list.
            return;
        }
        if ($tokens[$stackPtr]['code'] === \T_LIST) {
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
            if ($nextNonEmpty === \false || $tokens[$nextNonEmpty]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS || isset($tokens[$nextNonEmpty]['parenthesis_closer']) === \false) {
         ...
         */
        $hasVars = $phpcsFile->findNext(array(\T_VARIABLE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR), $opener + 1, $closer);
        if ($hasVars === \false) {
            // Empty list, not our concern.
            return $closer + 1;
        }
        // Set the variable delimiters based on the list type being examined.
        $stopPoints = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA);
        if ($tokens[$stackPtr]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY) {
            $stopPoints[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY;
        } else {
            $stopPoints[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS;
        }
        $listVars = array();
        ...
        do {
            ...
            $nextStartPoint = $phpcsFile->findNext(array(\T_VARIABLE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR), $lastStopPoint + 1, $nextStopPoint);
        ...
        if (\count($listVars) !== \count(\array_unique($listVars))) {
            $phpcsFile->addError('list() will assign variable from left-to-right since PHP 7.0. Ensure all variables in list() are unique to prevent unexpected results.', $stackPtr, 'Affected');
        }
        return $closer + 1;
    }
}
