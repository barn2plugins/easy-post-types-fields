...
class ForbiddenNamesSniff extends Sniff
{
     ...
     * @var array(string => string)
     */
    protected $validUseNames = array('const' => \true, 'function' => \true);
    /**
     ...
     * @var array
     */
    protected $targetedTokens = array(\T_CLASS, \T_FUNCTION, \T_NAMESPACE, \T_STRING, \T_CONST, \T_USE, \T_AS, \T_EXTENDS, \T_INTERFACE, \T_TRAIT);
    /**
    ...
    public function register()
    {
        ...
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_ANON_CLASS')) {
            $tokens[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS;
        }
        return $tokens;
    }
    /**
    ...
    public function processNonString(File $phpcsFile, $stackPtr, $tokens)
    {
        ...
        if ($tokens[$stackPtr]['type'] === 'T_ANON_CLASS' || $tokens[$stackPtr]['type'] === 'T_CLASS') {
        ...
        } elseif ($tokens[$stackPtr]['type'] === 'T_USE' && isset($this->validUseNames[\strtolower($tokens[$nextNonEmpty]['content'])]) === \true) {
            $maybeUseNext = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, null, \true, null, \true);
            if ($maybeUseNext !== \false && $this->isEndOfUseStatement($tokens[$maybeUseNext]) === \false) {
                $nextNonEmpty = $maybeUseNext;
            }
        } elseif ($tokens[$stackPtr]['type'] === 'T_AS' && isset($this->allowedModifiers[$tokens[$nextNonEmpty]['code']]) === \true && $phpcsFile->hasCondition($stackPtr, \T_USE) === \true) {
            $maybeUseNext = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, null, \true, null, \true);
            if ($maybeUseNext === \false || $this->isEndOfUseStatement($tokens[$maybeUseNext]) === \true) {
                return;
            }
            $nextNonEmpty = $maybeUseNext;
        } elseif ($tokens[$stackPtr]['type'] === 'T_AS' && isset($tokens[$stackPtr]['nested_parenthesis']) === \true && $tokens[$nextNonEmpty]['code'] === \T_LIST) {
            $parentheses = \array_reverse($tokens[$stackPtr]['nested_parenthesis'], \true);
        ...
        } elseif ($tokens[$stackPtr]['type'] === 'T_FUNCTION' && $tokens[$nextNonEmpty]['type'] === 'T_BITWISE_AND') {
            $maybeUseNext = $phpcsFile->findNext(Tokens::$emptyTokens, $nextNonEmpty + 1, null, \true, null, \true);
            ...
            $endToken = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET), $stackPtr + 1, null, \false, null, \true);
            $namespaceName = \trim($phpcsFile->getTokensAsString($stackPtr + 1, $endToken - $stackPtr - 1));
            if (empty($namespaceName) === \true) {
                return;
            }
         ...
         */
        if (($tokens[$stackPtr]['type'] === 'T_FUNCTION' && $this->inClassScope($phpcsFile, $stackPtr, \false) === \true || $tokens[$stackPtr]['type'] === 'T_CONST' && $this->isClassConstant($phpcsFile, $stackPtr) === \true && $nextContentLc !== 'class') && $this->supportsBelow('5.6') === \false) {
            return;
        }
        if ($this->supportsAbove($this->invalidNames[$nextContentLc])) {
            $data = array($tokens[$nextNonEmpty]['content'], $this->invalidNames[$nextContentLc]);
    ...
    public function processString(File $phpcsFile, $stackPtr, $tokens)
    {
        ...
        if (isset($this->invalidNames[$defineNameLc]) && $this->supportsAbove($this->invalidNames[$defineNameLc])) {
            $data = array($defineName, $this->invalidNames[$defineNameLc]);
    ...
    {
        return \in_array($token['code'], array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA), \true);
    }
}
