...
class ForbiddenNamesAsDeclaredSniff extends Sniff
{
     ...
     * @var array
     */
    protected $forbiddenTokens = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NULL => '7.0', \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_TRUE => '7.0', \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_FALSE => '7.0');
    /**
     ...
     * @var array
     */
    protected $forbiddenNames = array('null' => '7.0', 'true' => '7.0', 'false' => '7.0', 'bool' => '7.0', 'int' => '7.0', 'float' => '7.0', 'string' => '7.0', 'iterable' => '7.1', 'void' => '7.1', 'object' => '7.2');
    /**
     ...
     * @var array
     */
    protected $softReservedNames = array('resource' => '7.0', 'object' => '7.0', 'mixed' => '7.0', 'numeric' => '7.0');
    /**
    ...
    public function register()
    {
        ...
        $targets = array(\T_CLASS, \T_INTERFACE, \T_TRAIT, \T_NAMESPACE, \T_STRING);
        return $targets;
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (\in_array($tokenType, array('T_CLASS', 'T_INTERFACE', 'T_TRAIT'), \true)) {
            ...
            if ($nextNonEmpty !== \false && isset($this->forbiddenTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
            ...
            foreach ($namespaceParts as $namespacePart) {
                ...
                if (isset($this->allForbiddenNames[$partLc]) === \true) {
                    $name = $namespacePart;
                    $nameLc = $partLc;
                    break;
                }
            ...
            if ($nextNonEmptyCode !== \T_STRING && isset($this->forbiddenTokens[$nextNonEmptyCode]) === \true) {
                $name = $tokens[$nextNonEmpty]['content'];
                $nameLc = \strtolower($tokens[$nextNonEmpty]['content']);
            } elseif ($nextNonEmptyCode === \T_STRING) {
                $endOfStatement = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET), $stackPtr + 1);
                if ($endOfStatement === \false) {
                    return;
                }
                do {
                    $nextNonEmptyLc = \strtolower($tokens[$nextNonEmpty]['content']);
                    if (isset($this->allForbiddenNames[$nextNonEmptyLc]) === \true) {
                        $name = $tokens[$nextNonEmpty]['content'];
        ...
        $data = array($nameLc);
        if (isset($this->softReservedNames[$nameLc]) === \true && $this->supportsAbove($this->softReservedNames[$nameLc]) === \true) {
            $error .= ' soft reserved keyword as of PHP version %s';
            $isError = \false;
            $data[] = $this->softReservedNames[$nameLc];
        }
        if (isset($this->forbiddenNames[$nameLc]) === \true && $this->supportsAbove($this->forbiddenNames[$nameLc]) === \true) {
            if (isset($isError) === \true) {
                $error .= ' and a';
            }
