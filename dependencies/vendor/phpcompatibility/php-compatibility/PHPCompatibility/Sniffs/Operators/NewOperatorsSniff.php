...
class NewOperatorsSniff extends AbstractNewFeatureSniff
{
    ...
    protected $newOperators = array(
        'T_POW' => array('5.5' => \false, '5.6' => \true, 'description' => 'power operator (**)'),
        // Identified in PHP < 5.6 icw PHPCS < 2.4.0 as T_MULTIPLY + T_MULTIPLY.
        'T_POW_EQUAL' => array('5.5' => \false, '5.6' => \true, 'description' => 'power assignment operator (**=)'),
        // Identified in PHP < 5.6 icw PHPCS < 2.6.0 as T_MULTIPLY + T_MUL_EQUAL.
        'T_SPACESHIP' => array('5.6' => \false, '7.0' => \true, 'description' => 'spaceship operator (<=>)'),
        // Identified in PHP < 7.0 icw PHPCS < 2.5.1 as T_IS_SMALLER_OR_EQUAL + T_GREATER_THAN.
        'T_COALESCE' => array('5.6' => \false, '7.0' => \true, 'description' => 'null coalescing operator (??)'),
        // Identified in PHP < 7.0 icw PHPCS < 2.6.2 as T_INLINE_THEN + T_INLINE_THEN.
        'T_COALESCE_EQUAL' => array('7.3' => \false, '7.4' => \true, 'description' => 'null coalesce equal operator (??=)'),
    );
    /**
     ...
     */
    protected $newOperatorsPHPCSCompat = array('T_POW' => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MULTIPLY, 'T_POW_EQUAL' => \T_MUL_EQUAL, 'T_SPACESHIP' => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_GREATER_THAN, 'T_COALESCE' => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN, 'T_COALESCE_EQUAL' => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL);
    /**
     ...
     */
    protected $PHPCSCompatTranslate = array('T_MULTIPLY' => array('before' => 'T_MULTIPLY', 'real_token' => 'T_POW'), 'T_MUL_EQUAL' => array('before' => 'T_MULTIPLY', 'real_token' => 'T_POW_EQUAL'), 'T_GREATER_THAN' => array('before' => 'T_IS_SMALLER_OR_EQUAL', 'real_token' => 'T_SPACESHIP'), 'T_INLINE_THEN' => array('callback' => 'isTCoalesce', 'real_token' => 'T_COALESCE'), 'T_EQUAL' => array('callback' => 'isTCoalesceEqual', 'real_token' => 'T_COALESCE_EQUAL'));
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if (isset($this->newOperators[$tokenType]) === \false) {
            if (isset($this->PHPCSCompatTranslate[$tokenType]) && (isset($this->PHPCSCompatTranslate[$tokenType]['before'], $tokens[$stackPtr - 1]) === \true && $tokens[$stackPtr - 1]['type'] === $this->PHPCSCompatTranslate[$tokenType]['before'] || isset($this->PHPCSCompatTranslate[$tokenType]['callback']) === \true && \call_user_func(array($this, $this->PHPCSCompatTranslate[$tokenType]['callback']), $tokens, $stackPtr) === \true)) {
            ...
            if (isset($tokens[$stackPtr + 1]) !== \false && $tokens[$stackPtr + 1]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL) {
    ...
    {
        if ($tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL || isset($tokens[$stackPtr - 1]) === \false) {
        ...
        if ($tokens[$stackPtr - 1]['type'] === 'T_INLINE_THEN' && (isset($tokens[$stackPtr - 2]) && $tokens[$stackPtr - 2]['type'] === 'T_INLINE_THEN')) {
            return \true;
        }
        return \false;
    }
    ...
    {
        if ($tokens[$stackPtr]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN || isset($tokens[$stackPtr - 1]) === \false) {
            // Function called for wrong token or token has no predecessor.
            return \false;
        }
        if ($tokens[$stackPtr - 1]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN) {
