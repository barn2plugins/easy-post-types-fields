 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\FunctionNameRestrictions;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\Generic_Sniffs_NamingConventions_CamelCapsFunctionNameSniff as PHPCS_CamelCapsFunctionNameSniff;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_File as File;
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer_Standards_AbstractScopeSniff as PHPCS_AbstractScopeSniff;
...
class ReservedFunctionNamesSniff extends PHPCS_CamelCapsFunctionNameSniff
{
    ...
    public function __construct()
    {
        ...
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_ANON_CLASS')) {
            $scopeTokens[] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS;
        }
        // Call the grand-parent constructor directly.
        PHPCS_AbstractScopeSniff::__construct($scopeTokens, array(\T_FUNCTION), \true);
        // Make sure debuginfo is included in the array. Upstream only includes it since 2.5.1.
        $this->magicMethods['debuginfo'] = \true;
    }
    /**
    ...
    protected function processTokenWithinScope(File $phpcsFile, $stackPtr, $currScope)
    {
        ...
        if ($this->isFunctionDeprecated($phpcsFile, $stackPtr) === \true) {
            /*
        ...
        if (\preg_match('|^__[^_]|', $methodName) > 0) {
            ...
            if (isset($this->magicMethods[$magicPart]) === \false && isset($this->methodsDoubleUnderscore[$magicPart]) === \false) {
                ...
                $scopeNextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $currScope + 1, null, \true);
                if ($scopeNextNonEmpty !== \false && $tokens[$scopeNextNonEmpty]['code'] === \T_STRING) {
                    $className = $tokens[$scopeNextNonEmpty]['content'];
                }
                $phpcsFile->addWarning('Method name "%s" is discouraged; PHP has reserved all method names with a double underscore prefix for future use.', $stackPtr, 'MethodDoubleUnderscore', array($className . '::' . $methodName));
            }
        }
    }
    /**
    ...
    protected function processTokenOutsideScope(File $phpcsFile, $stackPtr)
    {
        ...
        if (\preg_match('|^__[^_]|', $functionName) > 0) {
            ...
            if (isset($this->magicFunctions[$magicPart]) === \false) {
                $phpcsFile->addWarning('Function name "%s" is discouraged; PHP has reserved all method names with a double underscore prefix for future use.', $stackPtr, 'FunctionDoubleUnderscore', array($functionName));
            }
        }
    }
    /**
    ...
    private function isFunctionDeprecated(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$commentEnd]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_CLOSE_TAG) {
