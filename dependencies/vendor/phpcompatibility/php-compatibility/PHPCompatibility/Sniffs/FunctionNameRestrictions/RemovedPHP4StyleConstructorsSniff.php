...
class RemovedPHP4StyleConstructorsSniff extends Sniff
{
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        $oldConstructorPos = -1;
        while (($nextFunc = $phpcsFile->findNext(array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_OPEN_TAG), $nextFunc + 1, $scopeCloser)) !== \false) {
            // Skip over docblocks.
            if ($tokens[$nextFunc]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_OPEN_TAG) {
            ...
            if ($funcNameLc === '__construct') {
                $newConstructorFound = \true;
            }
            if ($funcNameLc === $classNameLc) {
                $oldConstructorFound = \true;
                $oldConstructorPos = $nextFunc;
            }
            // If both have been found, no need to continue looping through the functions.
            if ($newConstructorFound === \true && $oldConstructorFound === \true) {
                break;
            }
            $nextFunc = $functionScopeCloser;
        }
        if ($newConstructorFound === \false && $oldConstructorFound === \true) {
            $phpcsFile->addWarning('Use of deprecated PHP4 style class constructor is not supported since PHP 7.', $oldConstructorPos, 'Found');
        }
    }
}
