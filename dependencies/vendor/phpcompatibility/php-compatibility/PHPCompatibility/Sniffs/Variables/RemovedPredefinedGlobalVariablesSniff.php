...
class RemovedPredefinedGlobalVariablesSniff extends AbstractRemovedFeatureSniff
{
     ...
     */
    protected $removedGlobalVariables = array('HTTP_POST_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_POST'), 'HTTP_GET_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_GET'), 'HTTP_ENV_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_ENV'), 'HTTP_SERVER_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_SERVER'), 'HTTP_COOKIE_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_COOKIE'), 'HTTP_SESSION_VARS' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_SESSION'), 'HTTP_POST_FILES' => array('5.3' => \false, '5.4' => \true, 'alternative' => '$_FILES'), 'HTTP_RAW_POST_DATA' => array('5.6' => \false, '7.0' => \true, 'alternative' => 'php://input'), 'php_errormsg' => array('7.2' => \false, 'alternative' => 'error_get_last()'));
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($this->isClassProperty($phpcsFile, $stackPtr) === \true) {
        ...
        if ($this->inClassScope($phpcsFile, $stackPtr, \false) === \true) {
            $prevToken = $phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
            if ($prevToken !== \false && $tokens[$prevToken]['code'] === \T_DOUBLE_COLON) {
                return;
            }
        }
        // Do some additional checks for the $php_errormsg variable.
        if ($varName === 'php_errormsg' && $this->isTargetPHPErrormsgVar($phpcsFile, $stackPtr, $tokens) === \false) {
            return;
        }
        // Still here, so throw an error/warning.
        $itemInfo = array('name' => $varName);
    ...
    private function isTargetPHPErrormsgVar(File $phpcsFile, $stackPtr, array $tokens)
    {
         ...
         */
        $function = $phpcsFile->getCondition($stackPtr, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
        if ($function === \false) {
            $function = $phpcsFile->getCondition($stackPtr, \T_FUNCTION);
        }
        // It could also be a function param, which is not in the function scope.
        if ($function === \false && isset($tokens[$stackPtr]['nested_parenthesis']) === \true) {
            $nestedParentheses = $tokens[$stackPtr]['nested_parenthesis'];
            $parenthesisCloser = \end($nestedParentheses);
            if (isset($tokens[$parenthesisCloser]['parenthesis_owner']) && ($tokens[$tokens[$parenthesisCloser]['parenthesis_owner']]['code'] === \T_FUNCTION || $tokens[$tokens[$parenthesisCloser]['parenthesis_owner']]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE)) {
        ...
        if ($nextNonEmpty !== \false && $tokens[$nextNonEmpty]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET) {
        ...
        if ($nextNonEmpty !== \false && isset(Tokens::$assignmentTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
            return \false;
        }
        // Is this a function param shadowing the PHP native one ?
        if ($function !== \false) {
            $parameters = PHPCSHelper::getMethodParameters($phpcsFile, $function);
            if (\is_array($parameters) === \true && empty($parameters) === \false) {
        ...
        $skipPast = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_INTERFACE' => \true, 'T_TRAIT' => \true, 'T_FUNCTION' => \true, 'T_CLOSURE' => \true);
        // Walk back and see if there is an assignment to the variable within the same scope.
        for ($i = $stackPtr - 1; $i >= $scopeStart; $i--) {
            if ($tokens[$i]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET && isset($tokens[$i]['scope_condition']) && isset($skipPast[$tokens[$tokens[$i]['scope_condition']]['type']])) {
            ...
            $nextNonEmpty = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
            if ($nextNonEmpty !== \false && isset(Tokens::$assignmentTokens[$tokens[$nextNonEmpty]['code']]) === \true) {
                return \false;
            }
        }
