 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\Variables;
...
class ForbiddenThisUseContextsSniff extends Sniff
{
     ...
     * @var array
     */
    private $ooScopeTokens = array('T_CLASS' => \T_CLASS, 'T_INTERFACE' => \T_INTERFACE, 'T_TRAIT' => \T_TRAIT);
    /**
     ...
     * @var array
     */
    private $skipOverScopes = array('T_FUNCTION' => \true, 'T_CLOSURE' => \true);
    /**
     ...
     * @var array
     */
    private $validUseOutsideObject = array(\T_ISSET => \true, \T_EMPTY => \true);
    /**
    ...
    public function register()
    {
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_ANON_CLASS')) {
            $this->ooScopeTokens['T_ANON_CLASS'] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS;
        }
        $this->skipOverScopes += $this->ooScopeTokens;
        return array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE, \T_GLOBAL, \T_CATCH, \T_FOREACH, \T_UNSET);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        switch ($tokens[$stackPtr]['code']) {
            case \T_FUNCTION:
                ...
                break;
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE:
                $this->isThisUsedAsParameter($phpcsFile, $stackPtr);
                break;
            case \T_GLOBAL:
                /*
                ...
                for ($i = $stackPtr + 1; $i < $endOfStatement; $i++) {
                    ...
                    $phpcsFile->addError('"$this" can no longer be used with the "global" keyword since PHP 7.1.', $i, 'Global');
                }
                break;
            case \T_CATCH:
                /*
                ...
                $varPtr = $phpcsFile->findNext(\T_VARIABLE, $tokens[$stackPtr]['parenthesis_opener'] + 1, $tokens[$stackPtr]['parenthesis_closer']);
                if ($varPtr === \false || $tokens[$varPtr]['content'] !== '$this') {
                    return;
                }
                $phpcsFile->addError('"$this" can no longer be used as a catch variable since PHP 7.1.', $varPtr, 'Catch');
                break;
            case \T_FOREACH:
                /*
                ...
                $stopPtr = $phpcsFile->findPrevious(array(\T_AS, \T_DOUBLE_ARROW), $tokens[$stackPtr]['parenthesis_closer'] - 1, $tokens[$stackPtr]['parenthesis_opener']);
                if ($stopPtr === \false) {
                    return;
                }
                $valueVarPtr = $phpcsFile->findNext(\T_VARIABLE, $stopPtr + 1, $tokens[$stackPtr]['parenthesis_closer']);
                if ($valueVarPtr === \false || $tokens[$valueVarPtr]['content'] !== '$this') {
                    return;
                }
                $afterThis = $phpcsFile->findNext(Tokens::$emptyTokens, $valueVarPtr + 1, $tokens[$stackPtr]['parenthesis_closer'], \true);
                if ($afterThis !== \false && ($tokens[$afterThis]['code'] === \T_OBJECT_OPERATOR || $tokens[$afterThis]['code'] === \T_DOUBLE_COLON)) {
                    return;
                }
                $phpcsFile->addError('"$this" can no longer be used as value variable in a foreach control structure since PHP 7.1.', $valueVarPtr, 'ForeachValueVar');
                break;
            case \T_UNSET:
                /*
                 * $this can no longer be unset.
                 */
                $openParenthesis = $phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
                if ($openParenthesis === \false || $tokens[$openParenthesis]['code'] !== \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS || isset($tokens[$openParenthesis]['parenthesis_closer']) === \false) {
                    return;
                }
                for ($i = $openParenthesis + 1; $i < $tokens[$openParenthesis]['parenthesis_closer']; $i++) {
                    ...
                    $afterThis = $phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, $tokens[$openParenthesis]['parenthesis_closer'], \true);
                    if ($afterThis !== \false && ($tokens[$afterThis]['code'] === \T_OBJECT_OPERATOR || $tokens[$afterThis]['code'] === \T_DOUBLE_COLON || $tokens[$afterThis]['code'] === \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET)) {
                        $i = $afterThis;
                        continue;
                    }
                    $phpcsFile->addError('"$this" can no longer be unset since PHP 7.1.', $i, 'Unset');
                }
                break;
        }
    }
    ...
    protected function isThisUsedAsParameter(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($params as $param) {
            ...
            if ($tokens[$stackPtr]['code'] === \T_FUNCTION) {
                $phpcsFile->addError('"$this" can no longer be used as a parameter since PHP 7.1.', $param['token'], 'FunctionParam');
            } else {
                $phpcsFile->addError('"$this" can no longer be used as a closure parameter since PHP 7.0.7.', $param['token'], 'ClosureParam');
            }
        }
    }
    /**
    ...
    protected function isThisUsedOutsideObjectContext(File $phpcsFile, $stackPtr)
    {
        ...
        for ($i = $tokens[$stackPtr]['scope_opener'] + 1; $i < $tokens[$stackPtr]['scope_closer']; $i++) {
            ...
            if (isset($tokens[$i]['nested_parenthesis']) === \true) {
                ...
                $previousNonEmpty = $phpcsFile->findPrevious(Tokens::$emptyTokens, $lastOpenParenthesis - 1, null, \true, null, \true);
                if (isset($this->validUseOutsideObject[$tokens[$previousNonEmpty]['code']])) {
                    continue;
                }
            }
            $phpcsFile->addError('"$this" can no longer be used in a plain function or method since PHP 7.1.', $i, 'OutsideObjectContext');
        }
    }
}
