 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\TextStrings;
...
class NewUnicodeEscapeSequenceSniff extends Sniff
{
    ...
    {
        return array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        foreach ($matches as $match) {
            $valid = \false;
            // If the close curly is missing, we have an incomplete escape sequence.
            if (isset($match[2])) {
                $valid = $this->isValidUnicodeEscapeSequence($match[1]);
            }
            if ($this->supportsBelow('5.6') === \true && $valid === \true) {
                $phpcsFile->addError('Unicode codepoint escape sequences are not supported in PHP 5.6 or earlier. Found: %s', $stackPtr, 'Found', array($match[0]));
            }
            if ($this->supportsAbove('7.0') === \true && $valid === \false) {
                $phpcsFile->addError('Strings containing a literal \\u{ followed by an invalid unicode codepoint escape sequence will cause a fatal error in PHP 7.0 and above. Escape the leading backslash to prevent this. Found: %s', $stackPtr, 'Invalid', array($match[0]));
            }
        }
    }
    /**
