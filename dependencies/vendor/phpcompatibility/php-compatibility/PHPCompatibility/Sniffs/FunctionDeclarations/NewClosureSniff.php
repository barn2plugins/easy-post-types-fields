 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHPCompatibility\Sniffs\FunctionDeclarations;
...
class NewClosureSniff extends Sniff
{
    ...
    {
        return array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
    }
    /**
    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        if ($this->supportsBelow('5.2')) {
            $phpcsFile->addError('Closures / anonymous functions are not available in PHP 5.2 or earlier', $stackPtr, 'Found');
        }
        /*
         * Closures can only be declared as static since PHP 5.4.
         */
        $isStatic = $this->isClosureStatic($phpcsFile, $stackPtr);
        if ($this->supportsBelow('5.3') && $isStatic === \true) {
            $phpcsFile->addError('Closures / anonymous functions could not be declared as static in PHP 5.3 or earlier', $stackPtr, 'StaticFound');
        }
        ...
        if ($this->supportsBelow('5.3')) {
            ...
            if ($usesThis !== \false) {
                $thisFound = $usesThis;
                do {
                    $phpcsFile->addError('Closures / anonymous functions did not have access to $this in PHP 5.3 or earlier', $thisFound, 'ThisFound');
            ...
            if ($usesClassRef !== \false) {
                do {
                    $phpcsFile->addError('Closures / anonymous functions could not use "%s::" in PHP 5.3 or earlier', $usesClassRef, 'ClassRefFound', array(\strtolower($tokens[$usesClassRef]['content'])));
        ...
        if ($this->supportsAbove('5.4') && $usesThis !== \false) {
            ...
            do {
                 ...
                 */
                if ($isStatic === \true) {
                    $phpcsFile->addError('Closures / anonymous functions declared as static do not have access to $this', $thisFound, 'ThisFoundInStatic');
                } elseif ($this->inClassScope($phpcsFile, $stackPtr, \false) === \false) {
                    $phpcsFile->addWarning('Closures / anonymous functions only have access to $this if used within a class or when bound to an object using bindTo(). Please verify.', $thisFound, 'ThisFoundOutsideClass');
                }
                $thisFound = $this->findThisUsageInClosure($phpcsFile, $thisFound + 1, $scopeEnd);
            } while ($thisFound !== \false);
        }
    ...
    protected function findThisUsageInClosure(File $phpcsFile, $startToken, $endToken)
    {
        ...
        return $phpcsFile->findNext(\T_VARIABLE, $startToken, $endToken, \false, '$this');
    }
    /**
    ...
    protected function findClassRefUsageInClosure(File $phpcsFile, $startToken, $endToken)
    {
        ...
        $tokens = $phpcsFile->getTokens();
        $classRef = $phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT, \T_STATIC), $startToken, $endToken);
        if ($classRef === \false || $tokens[$classRef]['code'] !== \T_STATIC) {
            return $classRef;
        }
        // T_STATIC, make sure it is used as a class reference.
        $next = $phpcsFile->findNext(Tokens::$emptyTokens, $classRef + 1, $endToken, \true);
