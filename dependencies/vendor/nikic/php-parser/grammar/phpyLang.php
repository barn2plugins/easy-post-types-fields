...
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = resolveNodes($matches['params']);
        $params = magicSplit('(?:' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PARAMS . '|' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = magicSplit('(?:' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PARAMS . '|' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
...
{
    return '~' . \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\LIB . '(?:' . \str_replace('~', '\\~', $regex) . ')~';
}
