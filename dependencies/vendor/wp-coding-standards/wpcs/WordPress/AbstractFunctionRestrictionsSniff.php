...
abstract class AbstractFunctionRestrictionsSniff extends Sniff
{
    ...
    public function is_targetted_token($stackPtr)
    {
        ...
        if ($this->is_class_object_call($stackPtr) === \true) {
            return \false;
        }
        ...
        if (\false !== $prev) {
            ...
            $skipped = array(\T_FUNCTION => \T_FUNCTION, \T_CLASS => \T_CLASS, \T_AS => \T_AS);
        ...
        if (\T_STRING === $this->tokens[$prev]['code'] && 'function' === $this->tokens[$prev]['content'] && (\T_AS === $this->tokens[$next]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON === $this->tokens[$next]['code'])) {
            return \true;
        }
    ...
    {
        $this->addMessage($this->groups[$group_name]['message'], $stackPtr, 'error' === $this->groups[$group_name]['type'], $this->string_to_errorcode($group_name . '_' . $matched_content), array($matched_content));
    }
    /**
