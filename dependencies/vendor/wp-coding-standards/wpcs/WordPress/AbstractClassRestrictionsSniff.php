    ...
    */
    ...
    public function is_targetted_token($stackPtr)
    {
        ...
        if (\in_array($token['code'], array(\T_NEW, \T_EXTENDS, \T_IMPLEMENTS), \true)) {
            if (\T_NEW === $token['code']) {
                $nameEnd = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS, \T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_OBJECT_OPERATOR), $stackPtr + 2) - 1;
            } else {
                $nameEnd = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET, \T_WHITESPACE), $stackPtr + 2) - 1;
            }
        ...
        if (\T_DOUBLE_COLON === $token['code']) {
            ...
            $nameStart = $this->phpcsFile->findPrevious(array(\T_STRING, \T_NS_SEPARATOR, \T_NAMESPACE), $nameEnd - 1, null, \true, null, \true) + 1;
        ...
        if (\in_array($classname, array('parent', 'self', 'static'), \true)) {
            return \false;
        }
