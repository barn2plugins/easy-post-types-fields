...
class AlternativeFunctionsSniff extends AbstractFunctionRestrictionsSniff
{
     ...
     * @var array
     */
    protected $allowed_local_streams = array('php://input' => \true, 'php://output' => \true, 'php://stdin' => \true, 'php://stdout' => \true, 'php://stderr' => \true);
    /**
     ...
     * @var array
     */
    protected $allowed_local_stream_constants = array('STDIN' => \true, 'STDOUT' => \true, 'STDERR' => \true);
    /**
    ...
    public function process_matched_token($stackPtr, $group_name, $matched_content)
    {
        ...
        switch ($matched_content) {
            ...
            case 'wp_parse_url':
                 ...
                 */
                if ($this->get_function_call_parameter_count($stackPtr) !== 1 && \version_compare($this->minimum_supported_version, '4.7.0', '<')) {
                    return;
                }
                break;
            case 'file_get_contents':
                ...
                if (\strpos($params[1]['raw'], 'http:') !== \false || \strpos($params[1]['raw'], 'https:') !== \false) {
                ...
                if ($this->is_local_data_stream($params[1]['raw']) === \true) {
                    // Local data stream.
                    return;
                }
                unset($params);
            ...
            case 'file_put_contents':
                ...
                if ($this->is_local_data_stream($first_param['raw']) === \true) {
                    // Local data stream.
                    return;
                }
                unset($first_param);
    ...
    protected function is_local_data_stream($raw_param_value)
    {
        ...
        if (isset($this->allowed_local_streams[$raw_stripped]) || isset($this->allowed_local_stream_constants[$raw_param_value])) {
            return \true;
        }
