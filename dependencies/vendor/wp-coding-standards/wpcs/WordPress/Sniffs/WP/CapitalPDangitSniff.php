...
class CapitalPDangitSniff extends Sniff
{
     ...
     * @var array
     */
    private $text_string_tokens = array(\T_CONSTANT_ENCAPSED_STRING => \T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_NOWDOC, \T_INLINE_HTML => \T_INLINE_HTML);
    /**
     * Comment tokens we want to listen for as they contain text strings.
     *
     * @var array
     */
    private $comment_text_tokens = array(\T_DOC_COMMENT => \T_DOC_COMMENT, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING, \T_COMMENT => \T_COMMENT);
    /**
    ...
    public function register()
    {
        ...
        $targets[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY;
        return $targets;
    }
    /**
    ...
    public function process_token($stackPtr)
    {
         ...
         */
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] && isset($this->tokens[$stackPtr]['bracket_closer'])) {
        ...
        if (isset(Tokens::$ooScopeTokens[$this->tokens[$stackPtr]['code']])) {
            ...
            if (\preg_match_all(self::WP_CLASSNAME_REGEX, $classname, $matches, \PREG_PATTERN_ORDER) > 0) {
                ...
                if (!empty($mispelled)) {
                    $this->phpcsFile->addWarning('Please spell "WordPress" correctly. Found: "%s" as part of the class/interface/trait name.', $stackPtr, 'MisspelledClassName', array(\implode(', ', $mispelled)));
                }
            }
            return;
        }
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING === $this->tokens[$stackPtr]['code'] || \T_DOC_COMMENT === $this->tokens[$stackPtr]['code']) {
            $comment_start = $this->phpcsFile->findPrevious(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_OPEN_TAG, $stackPtr - 1);
            if (\false !== $comment_start) {
                $comment_tag = $this->phpcsFile->findPrevious(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_TAG, $stackPtr - 1, $comment_start);
        ...
        if (\T_CONSTANT_ENCAPSED_STRING === $this->tokens[$stackPtr]['code']) {
            $prevToken = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
            if (\false !== $prevToken && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET === $this->tokens[$prevToken]['code']) {
                $nextToken = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
                if (\false !== $nextToken && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET === $this->tokens[$nextToken]['code']) {
                    return;
                }
            }
        }
        // Ignore constant declarations via define().
        if ($this->is_in_function_call($stackPtr, array('define' => \true), \true, \true)) {
            return;
        }
        // Ignore constant declarations using the const keyword.
        $stop_points = array(\T_CONST, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_OPEN_TAG, \T_CLOSE_TAG, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET);
        ...
        if (\preg_match_all(self::WP_REGEX, $content, $matches, \PREG_PATTERN_ORDER | \PREG_OFFSET_CAPTURE) > 0) {
            ...
            $fix = $this->phpcsFile->addFixableWarning('Please spell "WordPress" correctly. Found %s misspelling(s): %s', $stackPtr, 'Misspelled', array(\count($mispelled), \implode(', ', $mispelled)));
            if (\true === $fix) {
