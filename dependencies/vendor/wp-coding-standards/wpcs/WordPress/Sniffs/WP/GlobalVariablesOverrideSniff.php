...
class GlobalVariablesOverrideSniff extends Sniff
{
     ...
     * @var array
     */
    private $skip_over = array(\T_FUNCTION => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE => \true);
    /**
    ...
    public function register()
    {
        ...
        $targets = array(\T_GLOBAL, \T_VARIABLE, \T_LIST, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY);
    ...
    public function process_token($stackPtr)
    {
        ...
        if (isset(Tokens::$ooScopeTokens[$token['code']])) {
            if (\true === $this->is_test_class($stackPtr) && $token['scope_condition'] === $stackPtr && isset($token['scope_closer'])) {
         ...
         */
        $in_function_scope = $this->phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE));
        if ((\T_LIST === $token['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $token['code']) && \false === $in_function_scope && \false === $this->treat_files_as_scoped) {
    ...
    {
        if ($this->has_whitelist_comment('override', $stackPtr) === \true) {
            return;
        }
        $token = $this->tokens[$stackPtr];
        ...
        if ('GLOBALS' === $var_name) {
            ...
            if (\false === $bracketPtr || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET !== $this->tokens[$bracketPtr]['code'] || !isset($this->tokens[$bracketPtr]['bracket_closer'])) {
                return;
            }
            ...
            for ($ptr = $start; $ptr < $this->tokens[$bracketPtr]['bracket_closer']; $ptr++) {
                 ...
                 */
                if (\T_VARIABLE === $this->tokens[$ptr]['code'] || \T_STRING === $this->tokens[$ptr]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING === $this->tokens[$ptr]['code']) {
                    return;
                }
    ...
    protected function process_global_statement($stackPtr, $in_function_scope)
    {
        ...
        while (isset($this->tokens[$ptr])) {
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON === $var['code']) {
                break;
            }
            if (\T_VARIABLE === $var['code']) {
                $var_name = \substr($var['content'], 1);
                if (isset($this->wp_globals[$var_name]) && isset($this->override_allowed[$var_name]) === \false) {
                    $search[] = $var['content'];
                }
            }
            $ptr++;
        ...
        if (\true === $in_function_scope) {
            ...
            $closure_cond = $this->phpcsFile->getCondition($stackPtr, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
            $scope_cond = \max($function_cond, $closure_cond);
        ...
        for ($ptr = $start; $ptr < $end; $ptr++) {
            ...
            if (\T_LIST === $this->tokens[$ptr]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$ptr]['code']) {
                ...
                foreach ($var_pointers as $ptr) {
                    ...
                    if (\in_array($var_name, $search, \true)) {
                        $this->process_variable_assignment($ptr, \true);
                    }
                }
                // No need to re-examine these variables.
