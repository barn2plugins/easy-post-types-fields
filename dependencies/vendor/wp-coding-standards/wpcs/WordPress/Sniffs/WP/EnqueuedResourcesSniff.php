...
class EnqueuedResourcesSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\preg_match('# rel=\\\\?[\'"]?stylesheet\\\\?[\'"]?#', $token['content']) > 0) {
            $this->phpcsFile->addError('Stylesheets must be registered/enqueued via wp_enqueue_style', $stackPtr, 'NonEnqueuedStylesheet');
        }
        if (\preg_match('#<script[^>]*(?<=src=)#', $token['content']) > 0) {
            $this->phpcsFile->addError('Scripts must be registered/enqueued via wp_enqueue_script', $stackPtr, 'NonEnqueuedScript');
        }
    }
}
