 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\WP;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\AbstractFunctionRestrictionsSniff;
...
class I18nSniff extends AbstractFunctionRestrictionsSniff
{
     ...
     * @var bool
     */
    public $check_translator_comments = \true;
    /**
    ...
    public function process_token($stack_ptr)
    {
        ...
        $cl_text_domain = \trim(PHPCSHelper::get_config_data('text_domain'));
    ...
    {
        $func_open_paren_token = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stack_ptr + 1, null, \true);
        if (\false === $func_open_paren_token || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$func_open_paren_token]['code'] || !isset($this->tokens[$func_open_paren_token]['parenthesis_closer'])) {
        ...
        for ($i = $func_open_paren_token + 1; $i < $this->tokens[$func_open_paren_token]['parenthesis_closer']; $i++) {
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA === $this_token['code']) {
         ...
         */
        if (\false !== \strpos($this->i18n_functions[$matched_content], 'number') && isset($argument_assertions[0]['tokens'], $argument_assertions[1]['tokens']) && \count($argument_assertions[0]['tokens']) === 1 && \count($argument_assertions[1]['tokens']) === 1) {
        ...
        if (\true === $this->check_translator_comments) {
    ...
    protected function check_argument_tokens($context)
    {
        ...
        if (empty($tokens) || 0 === \count($tokens)) {
            ...
            if (\true === $this->text_domain_contains_default) {
                $this->phpcsFile->addWarning('Missing $%s arg. If this text string is supposed to use a WP Core translation, use the "default" text domain.', $stack_ptr, $code . 'Default', array($arg_name));
        ...
        if (\in_array($arg_name, array('text', 'single', 'plural'), \true)) {
            $this->check_text($context);
        }
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING === $tokens[0]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC === $tokens[0]['code']) {
        ...
        if (isset(Tokens::$textStringTokens[$tokens[0]['code']])) {
            if ('domain' === $arg_name && !empty($this->text_domain)) {
                ...
                if (!\in_array($stripped_content, $this->text_domain, \true)) {
                    $this->addMessage('Mismatched text domain. Expected \'%s\' but got %s.', $stack_ptr, $is_error, 'TextDomainMismatch', array(\implode("' or '", $this->text_domain), $content));
                    return \false;
                }
                if (\true === $this->text_domain_is_default && 'default' === $stripped_content) {
                    ...
                    if ($tokens[0]['token_index'] === $stack_ptr) {
                        $prev = $this->phpcsFile->findPrevious(\T_WHITESPACE, $stack_ptr - 1, null, \true);
                        if (\false !== $prev && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA === $this->tokens[$prev]['code']) {
                            $fixable = \true;
                        }
                    }
                    if (\false === $fixable) {
    ...
    protected function check_text($context)
    {
        ...
        if ($unordered_matches_count > 0 && $unordered_matches_count !== $all_matches_count && $all_matches_count > 1) {
            ...
            $this->phpcsFile->addError('Multiple placeholders should be ordered. Mix of ordered and non-ordered placeholders found. Found: %s.', $stack_ptr, $code, array(\implode(', ', $all_matches[0])));
            ...
            $fix = $this->addFixableMessage('Multiple placeholders should be ordered. Expected \'%s\', but got %s.', $stack_ptr, $is_error, $code, array(\implode(', ', $suggestions), \implode(', ', $unordered_matches)));
            if (\true === $fix) {
         ...
         */
        $content_without_quotes = \trim($this->strip_quotes($content));
    ...
    {
        foreach ($args as $arg) {
            if (\false === \in_array($arg['arg_name'], array('text', 'single', 'plural'), \true)) {
                continue;
            }
            if (empty($arg['tokens'])) {
                continue;
            ...
            foreach ($arg['tokens'] as $token) {
                ...
                if (\false !== $previous_comment) {
                    ...
                    if ($this->tokens[$previous_comment]['line'] + 1 === $this->tokens[$stack_ptr]['line']) {
                        ...
                        $next_non_whitespace = $this->phpcsFile->findNext(\T_WHITESPACE, $previous_comment + 1, $stack_ptr, \true);
                    ...
                    if (\true === $correctly_placed) {
                        ...
                        } elseif (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_CLOSE_TAG === $this->tokens[$previous_comment]['code']) {
                            // If it's docblock comment (wrong style) make sure that it's a translators comment.
                            $db_start = $this->phpcsFile->findPrevious(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_OPEN_TAG, $previous_comment - 1);
                            $db_first_text = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING, $db_start + 1, $previous_comment);
                            if (\true === $this->is_translators_comment($this->tokens[$db_first_text]['content'])) {
                                $this->phpcsFile->addWarning('A "translators:" comment must be a "/* */" style comment. Docblock comments will not be picked up by the tools to generate a ".pot" file.', $stack_ptr, 'TranslatorsCommentWrongStyle');
                                return;
                            }
                        }
                    }
                ...
                $this->phpcsFile->addWarning('A gettext call containing placeholders was found, but was not accompanied by a "translators:" comment on the line above to clarify the meaning of the placeholders.', $stack_ptr, 'MissingTranslatorsComment');
                return;
            }
        }
    }
