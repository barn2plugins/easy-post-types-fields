...
class CronIntervalSniff extends Sniff
{
    ...
    {
        return array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\false !== $callbackArrayPtr && (\T_ARRAY === $this->tokens[$callbackArrayPtr]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$callbackArrayPtr]['code'])) {
        ...
        $callbackFunctionPtr = $this->phpcsFile->findNext(array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE), $callback['start'], $callback['end'] + 1);
        if (\false === $callbackFunctionPtr) {
            $this->confused($stackPtr);
            return;
        }
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $this->tokens[$callbackFunctionPtr]['code']) {
        ...
        for ($i = $opening; $i <= $closing; $i++) {
            if (\in_array($this->tokens[$i]['code'], array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING), \true)) {
                if ('interval' === $this->strip_quotes($this->tokens[$i]['content'])) {
                    ...
                    $valueEnd = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS), $valueStart + 1);
                    $value = '';
