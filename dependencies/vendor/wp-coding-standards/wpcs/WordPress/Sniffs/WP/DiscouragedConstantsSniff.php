...
class DiscouragedConstantsSniff extends AbstractFunctionParameterSniff
{
     ...
     * @var array
     */
    private $preceding_tokens_to_ignore = array(\T_NAMESPACE => \true, \T_USE => \true, \T_CLASS => \true, \T_TRAIT => \true, \T_INTERFACE => \true, \T_EXTENDS => \true, \T_IMPLEMENTS => \true, \T_NEW => \true, \T_FUNCTION => \true, \T_DOUBLE_COLON => \true, \T_OBJECT_OPERATOR => \true, \T_INSTANCEOF => \true, \T_GOTO => \true);
    /**
    ...
    public function process_arbitrary_tstring($stackPtr)
    {
        ...
        if (\false !== $next && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS === $this->tokens[$next]['code']) {
        ...
        if (\false !== $prev && \T_CONST === $this->tokens[$prev]['code'] && \true === $this->is_class_constant($prev)) {
        ...
        $first_on_line = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
        if (\false !== $first_on_line && \T_USE === $this->tokens[$first_on_line]['code']) {
            $next_on_line = $this->phpcsFile->findNext(Tokens::$emptyTokens, $first_on_line + 1, null, \true);
            if (\false !== $next_on_line) {
                if (\T_STRING === $this->tokens[$next_on_line]['code'] && 'const' === $this->tokens[$next_on_line]['content'] || \T_CONST === $this->tokens[$next_on_line]['code']) {
        ...
        $this->phpcsFile->addWarning('Found usage of constant "%s". Use %s instead.', $stackPtr, $this->string_to_errorcode($content . 'UsageFound'), array($content, $this->discouraged_constants[$content]));
    }
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (isset($this->discouraged_constants[$raw_content])) {
            $this->phpcsFile->addWarning('Found declaration of constant "%s". Use %s instead.', $stackPtr, $this->string_to_errorcode($raw_content . 'DeclarationFound'), array($raw_content, $this->discouraged_constants[$raw_content]));
        }
    }
}
