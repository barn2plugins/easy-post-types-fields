...
class PreparedSQLPlaceholdersSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        for ($i = $query['start']; $i <= $query['end']; $i++) {
                    ...
                    if ('sprintf' === \strtolower($this->tokens[$i]['content'])) {
                        ...
                        if (\preg_match('`\\s+IN\\s*\\(\\s*(' . $regex_quote . ')?$`i', $prev_content, $match) > 0) {
                            ...
                            if ($this->analyse_implode($i) === \true) {
                                ...
                                if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS === $this->tokens[$next]['code'] && isset($this->tokens[$next]['parenthesis_closer'])) {
                                    $skip_from = $i + 1;
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING === $this->tokens[$i]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_HEREDOC === $this->tokens[$i]['code']) {
        ...
        $count_diff_whitelisted = $this->has_whitelist_comment('PreparedSQLPlaceholders replacement count', $stackPtr);
        ...
        if (isset($parameters[2]) && 2 === $total_parameters) {
            ...
            if (\false !== $next && (\T_ARRAY === $this->tokens[$next]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$next]['code'])) {
