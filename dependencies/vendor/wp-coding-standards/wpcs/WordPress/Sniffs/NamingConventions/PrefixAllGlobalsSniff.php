 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\NamingConventions;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\AbstractFunctionParameterSniff;
...
class PrefixAllGlobalsSniff extends AbstractFunctionParameterSniff
{
     ...
     * @var string[]
     */
    protected $prefix_blacklist = array('wordpress' => \true, 'wp' => \true, '_' => \true, 'php' => \true);
    /**
     ...
     * @var array
     */
    protected $superglobals = array('_COOKIE' => \true, '_ENV' => \true, '_GET' => \true, '_FILES' => \true, '_POST' => \true, '_REQUEST' => \true, '_SERVER' => \true, '_SESSION' => \true);
    /**
     ...
     * @var array
     */
    protected $whitelisted_core_hooks = array('widget_title' => \true, 'add_meta_boxes' => \true);
    /**
    ...
    public function register()
    {
        ...
        $targets = array(
            ...
            \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR,
            // Variable variables.
            \T_LIST => \T_LIST,
            \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY,
        );
    ...
    public function getGroups()
    {
        ...
        unset($this->target_functions['do_action_deprecated'], $this->target_functions['apply_filters_deprecated']);
    ...
    public function process_token($stackPtr)
    {
        ...
        $cl_prefixes = \trim(PHPCSHelper::get_config_data('prefixes'));
        ...
        // Ignore test classes.
        if (isset(Tokens::$ooScopeTokens[$this->tokens[$stackPtr]['code']]) && \true === $this->is_test_class($stackPtr)) {
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS === $this->tokens[$stackPtr]['code']) {
        ...
        if (\T_STRING === $this->tokens[$stackPtr]['code']) {
        ...
        } elseif (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOLLAR === $this->tokens[$stackPtr]['code']) {
        ...
        } elseif (\T_LIST === $this->tokens[$stackPtr]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code']) {
            ...
            switch ($this->tokens[$stackPtr]['type']) {
                case 'T_FUNCTION':
                    ...
                    if ($this->phpcsFile->hasCondition($stackPtr, Tokens::$ooScopeTokens) === \true) {
                        return;
                    }
                    if ($this->is_function_deprecated($this->phpcsFile, $stackPtr) === \true) {
                        /*
                ...
                case 'T_CONST':
                    ...
                    if (\true === $this->is_class_constant($stackPtr)) {
                        return;
                    }
    ...
    {
        static $indicators = array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET => \true, \T_VARIABLE => \true);
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET === $this->tokens[$next_non_empty]['code'] && isset($this->tokens[$next_non_empty]['bracket_closer'])) {
        ...
        while (\false !== $maybe_assignment && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET === $this->tokens[$maybe_assignment]['code'] && isset($this->tokens[$maybe_assignment]['bracket_closer'])) {
         ...
         */
        if ($this->phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE)) === \true) {
            $condition = $this->phpcsFile->getCondition($stackPtr, \T_FUNCTION);
            if (\false === $condition) {
                $condition = $this->phpcsFile->getCondition($stackPtr, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
            }
    ...
    protected function process_variable_assignment($stackPtr, $in_list = \false)
    {
         ...
         */
        if (\false === $in_list && \false === $this->is_assignment($stackPtr) && \false === $this->is_foreach_as($stackPtr)) {
            return;
        }
        $is_error = \true;
        ...
        if ($this->variable_prefixed_or_whitelisted($stackPtr, $variable_name) === \true) {
            return;
        }
        if ('GLOBALS' === $variable_name) {
            $array_open = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
            if (\false === $array_open || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET !== $this->tokens[$array_open]['code']) {
            ...
            if (isset(Tokens::$stringTokens[$this->tokens[$array_key]['code']]) && $this->variable_prefixed_or_whitelisted($stackPtr, $variable_name) === \true) {
                return;
            }
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING === $this->tokens[$array_key]['code']) {
                ...
                if ('' !== $first) {
                    if ($this->variable_prefixed_or_whitelisted($array_key, $first) === \true) {
                        return;
                    }
                } else {
            ...
            if (\false === $in_list && isset($this->tokens[$stackPtr]['nested_parenthesis'])) {
                foreach ($this->tokens[$stackPtr]['nested_parenthesis'] as $opener => $closer) {
                    if (isset($this->tokens[$opener]['parenthesis_owner']) && (\T_FUNCTION === $this->tokens[$this->tokens[$opener]['parenthesis_owner']]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $this->tokens[$this->tokens[$opener]['parenthesis_owner']]['code'])) {
                        return;
                    }
                }
                unset($opener, $closer);
            ...
            if (\false === $in_list && \true === $this->is_class_property($stackPtr)) {
                return;
            }
            ...
            if ($this->phpcsFile->hasCondition($stackPtr, array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE)) === \true) {
                ...
                $end_of_statement = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, $has_global + 1);
        ...
        // Still here ? In that case, the variable name should be prefixed.
        $recorded = $this->addMessage(self::ERROR_MSG, $stackPtr, $is_error, 'NonPrefixedVariableFound', array('Global variables defined', '$' . $variable_name));
        if (\true === $recorded) {
            $this->record_potential_prefix_metric($stackPtr, $variable_name);
        }
    }
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if ('define' !== $matched_content && isset($this->whitelisted_core_hooks[$raw_content]) || 'define' === $matched_content && isset($this->whitelisted_core_constants[$raw_content])) {
            return;
        }
        if ($this->is_prefixed($parameters[1]['start'], $raw_content) === \true) {
            return;
        } else {
            // This may be a dynamic hook/constant name.
            $first_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $parameters[1]['start'], $parameters[1]['end'] + 1, \true);
            ...
            if (isset(Tokens::$stringTokens[$this->tokens[$first_non_empty]['code']]) && $this->is_prefixed($parameters[1]['start'], $first_non_empty_content) === \true) {
                return;
            }
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING === $this->tokens[$first_non_empty]['code']) {
                // If the first part of the parameter is a double quoted string, try again with only
                // the part before the first variable (if any).
                $exploded = \explode('$', $first_non_empty_content);
                $first = \rtrim($exploded[0], '{');
                if ('' !== $first) {
                    if ($this->is_prefixed($parameters[1]['start'], $first) === \true) {
                        return;
                    }
                } else {
