...
class EscapedNotTranslatedSniff extends AbstractFunctionParameterSniff
{
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        $data = array($matched_content, $this->target_functions[$matched_content], $this->phpcsFile->getTokensAsString($stackPtr, $closer - $stackPtr + 1));
        $this->phpcsFile->addWarning('%s() expects only one parameter. Did you mean to use %s() ? Found: %s', $stackPtr, 'Found', $data);
    }
}
