...
class EmptyStatementSniff extends Sniff
{
    ...
    {
        return array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_CLOSE_TAG);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        switch ($this->tokens[$stackPtr]['type']) {
             ...
             */
            case 'T_SEMICOLON':
                $prevNonEmpty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
                if (\false === $prevNonEmpty || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON !== $this->tokens[$prevNonEmpty]['code'] && \T_OPEN_TAG !== $this->tokens[$prevNonEmpty]['code'] && \T_OPEN_TAG_WITH_ECHO !== $this->tokens[$prevNonEmpty]['code']) {
                    return;
                }
                if (isset($this->tokens[$stackPtr]['nested_parenthesis'])) {
                    $nested = $this->tokens[$stackPtr]['nested_parenthesis'];
                    $last_closer = \array_pop($nested);
                    if (isset($this->tokens[$last_closer]['parenthesis_owner']) && \T_FOR === $this->tokens[$this->tokens[$last_closer]['parenthesis_owner']]['code']) {
                        // Empty for() condition.
                        return;
                    }
                }
                $fix = $this->phpcsFile->addFixableWarning('Empty PHP statement detected: superfluous semi-colon.', $stackPtr, 'SemicolonWithoutCodeDetected');
                if (\true === $fix) {
                    $this->phpcsFile->fixer->beginChangeset();
                    if (\T_OPEN_TAG === $this->tokens[$prevNonEmpty]['code'] || \T_OPEN_TAG_WITH_ECHO === $this->tokens[$prevNonEmpty]['code']) {
                        /*
                    ...
                    for ($i = $stackPtr; $i > $prevNonEmpty; $i--) {
                        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON !== $this->tokens[$i]['code'] && \T_WHITESPACE !== $this->tokens[$i]['code']) {
                            break;
                        }
             ...
             */
            case 'T_CLOSE_TAG':
                $prevNonEmpty = $this->phpcsFile->findPrevious(\T_WHITESPACE, $stackPtr - 1, null, \true);
                if (\false === $prevNonEmpty || \T_OPEN_TAG !== $this->tokens[$prevNonEmpty]['code'] && \T_OPEN_TAG_WITH_ECHO !== $this->tokens[$prevNonEmpty]['code']) {
                    return;
                }
                $fix = $this->phpcsFile->addFixableWarning('Empty PHP open/close tag combination detected.', $prevNonEmpty, 'EmptyPHPOpenCloseTagsDetected');
                if (\true === $fix) {
                    $this->phpcsFile->fixer->beginChangeset();
