 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\Classes;
...
class ClassInstantiationSniff extends Sniff
{
    ...
    public function register()
    {
        ...
        $this->classname_tokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SELF;
        $this->classname_tokens[\T_STATIC] = \T_STATIC;
        $this->classname_tokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PARENT;
        $this->classname_tokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS;
        ...
        $this->classname_tokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET;
        $this->classname_tokens[\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET] = \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET;
        $this->classname_tokens[\T_CONSTANT_ENCAPSED_STRING] = \T_CONSTANT_ENCAPSED_STRING;
        $this->classname_tokens[\T_LNUMBER] = \T_LNUMBER;
        return array(\T_NEW, \T_STRING);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if ('PHP' === $this->phpcsFile->tokenizerType && \T_NEW !== $this->tokens[$stackPtr]['code'] || 'JS' === $this->phpcsFile->tokenizerType && (\T_STRING !== $this->tokens[$stackPtr]['code'] || 'new' !== \strtolower($this->tokens[$stackPtr]['content']))) {
            return;
        }
        /*
        ...
        if ('PHP' === $this->phpcsFile->tokenizerType) {
            $prev_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
            if (\false !== $prev_non_empty && 'T_BITWISE_AND' === $this->tokens[$prev_non_empty]['type']) {
                ...
                $this->phpcsFile->addError('Assigning the return value of new by reference is no longer supported by PHP.', $stackPtr, 'NewByReferenceFound');
            } else {
         ...
         */
        $next_non_empty_after_class_name = $this->phpcsFile->findNext($this->classname_tokens, $stackPtr + 1, null, \true, null, \true);
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$next_non_empty_after_class_name]['code']) {
            $this->phpcsFile->recordMetric($stackPtr, 'Object instantiation with parenthesis', 'no');
            $fix = $this->phpcsFile->addFixableError('Parenthesis should always be used when instantiating a new object.', $classname_ptr, 'MissingParenthesis');
            if (\true === $fix) {
            ...
            if ($next_non_empty_after_class_name - 1 !== $classname_ptr) {
                $this->phpcsFile->recordMetric($stackPtr, 'Space between classname and parenthesis', $next_non_empty_after_class_name - $classname_ptr);
