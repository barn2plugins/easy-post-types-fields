 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\Files;
...
class FileNameSniff extends Sniff
{
     ...
     * @var array
     */
    private $class_exceptions = array('class.wp-dependencies.php' => \true, 'class.wp-scripts.php' => \true, 'class.wp-styles.php' => \true);
    /**
     ...
     * @var array
     */
    private $unittest_class_exceptions = array('class.wp-dependencies.inc' => \true, 'class.wp-scripts.inc' => \true, 'class.wp-styles.inc' => \true);
    /**
    ...
    {
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\PHP_CODESNIFFER_IN_TESTS')) {
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_PHPCS_DISABLE') && \defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\T_PHPCS_ENABLE')) {
            $i = -1;
            while ($i = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PHPCS_DISABLE, $i + 1)) {
                if (empty($this->tokens[$i]['sniffCodes']) || isset($this->tokens[$i]['sniffCodes']['WordPress']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files']) || isset($this->tokens[$i]['sniffCodes']['WordPress.Files.FileName'])) {
                    do {
                        $i = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PHPCS_ENABLE, $i + 1);
        ...
        if ($fileName !== $expected && (\false === $this->is_theme || 1 !== \preg_match(self::THEME_EXCEPTIONS_REGEX, $fileName))) {
            $this->phpcsFile->addError('Filenames should be all lowercase with hyphens as word separators. Expected %s, but found %s.', 0, 'NotHyphenatedLowercase', array($expected, $fileName));
        }
        unset($expected);
        /*
         ...
         */
        if (\true === $this->strict_class_file_names) {
            ...
            if (\false !== $has_class && \false === $this->is_test_class($has_class)) {
                ...
                if (\substr($fileName, 0, -4) !== $expected && !isset($this->class_exceptions[$fileName])) {
                    $this->phpcsFile->addError('Class file names should be based on the class name with "class-" prepended. Expected %s, but found %s.', 0, 'InvalidClassFileName', array($expected . '.php', $fileName));
                }
                unset($expected);
            }
        }
        ...
        if (\false !== \strpos($file, \DIRECTORY_SEPARATOR . 'wp-includes' . \DIRECTORY_SEPARATOR)) {
            $subpackage_tag = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_TAG, $stackPtr, null, \false, '@subpackage');
            if (\false !== $subpackage_tag) {
                $subpackage = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING, $subpackage_tag);
                if (\false !== $subpackage) {
                    $fileName_end = \substr($fileName, -13);
                    $has_class = $this->phpcsFile->findNext(\T_CLASS, $stackPtr);
                    if ('Template' === \trim($this->tokens[$subpackage]['content']) && $this->tokens[$subpackage_tag]['line'] === $this->tokens[$subpackage]['line'] && (!\defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\PHP_CODESNIFFER_IN_TESTS') && '-template.php' !== $fileName_end || \defined('Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\PHP_CODESNIFFER_IN_TESTS') && '-template.inc' !== $fileName_end) && \false === $has_class) {
                        $this->phpcsFile->addError('Files containing template tags should have "-template" appended to the end of the file name. Expected %s, but found %s.', 0, 'InvalidTemplateTagFileName', array(\substr($fileName, 0, -4) . '-template.php', $fileName));
                    }
                }
            }
        }
