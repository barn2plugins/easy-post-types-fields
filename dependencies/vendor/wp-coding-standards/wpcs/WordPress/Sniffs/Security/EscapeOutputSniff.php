 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\Security;
...
class EscapeOutputSniff extends Sniff
{
     ...
     * @var array
     */
    protected $addedCustomFunctions = array('escape' => array(), 'autoescape' => array(), 'sanitize' => array(), 'print' => array());
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (!isset($end_of_statement)) {
            $end_of_statement = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON, \T_CLOSE_TAG), $stackPtr);
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$open_paren]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS !== $this->tokens[$last_token]['code']) {
                $ternary = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN, $stackPtr, $end_of_statement);
        ...
        for ($i = $stackPtr; $i < $end_of_statement; $i++) {
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS === $this->tokens[$i]['code']) {
                ...
                if ($in_cast) {
                    ...
                    $ternary = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_THEN, $i, $this->tokens[$i]['parenthesis_closer']);
                    if (\false !== $ternary) {
                        $next_paren = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS, $i + 1, $this->tokens[$i]['parenthesis_closer']);
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$i]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY === $this->tokens[$i]['code']) {
                continue;
            }
            if (\in_array($this->tokens[$i]['code'], array(\T_DOUBLE_ARROW, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS), \true)) {
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$i]['code'] && isset($this->safe_php_constants[$this->tokens[$i]['content']]) && $this->is_use_of_global_constant($i)) {
                continue;
            }
            // Wake up on concatenation characters, another part to check.
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_STRING_CONCAT === $this->tokens[$i]['code']) {
                $watch = \true;
                continue;
            }
            // Wake up after a ternary else (:).
            if (\false !== $ternary && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_INLINE_ELSE === $this->tokens[$i]['code']) {
                $watch = \true;
                continue;
            }
            // Wake up for commas.
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA === $this->tokens[$i]['code']) {
                $in_cast = \false;
                $watch = \true;
                continue;
            }
            ...
            if (\T_STRING === $this->tokens[$i]['code']) {
                ...
                $function_opener = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS, $i + 1, null, \false, null, \true);
                ...
                if (\false !== $function_opener) {
                    if (isset($this->arrayWalkingFunctions[$functionName])) {
                        ...
                        $callback = $this->get_function_call_parameter($ptr, $this->arrayWalkingFunctions[$functionName]);
                        if (!empty($callback)) {
                            /*
                ...
                if ($is_formatting_function || isset($this->autoEscapedFunctions[$functionName]) || isset($this->escapingFunctions[$functionName])) {
                    continue;
                }
    ...
    protected function mergeFunctionLists()
    {
        if ($this->customEscapingFunctions !== $this->addedCustomFunctions['escape']) {
            ...
            $this->escapingFunctions = $this->merge_custom_array($customEscapeFunctions, $this->escapingFunctions);
        ...
        if ($this->customAutoEscapedFunctions !== $this->addedCustomFunctions['autoescape']) {
            $this->autoEscapedFunctions = $this->merge_custom_array($this->customAutoEscapedFunctions, $this->autoEscapedFunctions);
        ...
        if ($this->customPrintingFunctions !== $this->addedCustomFunctions['print']) {
            $this->printingFunctions = $this->merge_custom_array($this->customPrintingFunctions, $this->printingFunctions);
