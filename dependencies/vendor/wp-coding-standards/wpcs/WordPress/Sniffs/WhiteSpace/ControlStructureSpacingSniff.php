 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\WhiteSpace;
...
class ControlStructureSpacingSniff extends Sniff
{
     ...
     * @var array
     */
    private $ignore_extra_space_after_open_paren = array(\T_FUNCTION => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE => \true, \T_DO => \true, \T_ELSE => \true, \T_TRY => \true);
    /**
    ...
    {
        return array(\T_IF, \T_WHILE, \T_FOREACH, \T_FOR, \T_SWITCH, \T_DO, \T_ELSE, \T_ELSEIF, \T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE, \T_USE, \T_TRY, \T_CATCH);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (isset($this->tokens[$stackPtr + 1]) && \T_WHITESPACE !== $this->tokens[$stackPtr + 1]['code'] && !(\T_ELSE === $this->tokens[$stackPtr]['code'] && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON === $this->tokens[$stackPtr + 1]['code']) && !(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 >= $this->spaces_before_closure_open_paren)) {
        ...
        if (!isset($this->tokens[$stackPtr]['scope_closer'])) {
            if (\T_USE === $this->tokens[$stackPtr]['code'] && 'closure' === $this->get_use_type($stackPtr)) {
                $scopeOpener = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET, $stackPtr + 1);
        ...
        // Alternative syntax.
        if (isset($scopeOpener) && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON === $this->tokens[$scopeOpener]['code']) {
        ...
        if (\T_FUNCTION === $this->tokens[$stackPtr]['code']) {
            ...
            } elseif (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_BITWISE_AND === $this->tokens[$parenthesisOpener]['code']) {
            ...
            if (isset($function_name_ptr)) {
                ...
                if ($function_name_ptr + 1 !== $parenthesisOpener) {
                    ...
                    $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'SpaceBeforeFunctionOpenParenthesis', $this->tokens[$function_name_ptr + 1]['content']);
                    if (\true === $fix) {
                        $this->phpcsFile->fixer->replaceToken($function_name_ptr + 1, '');
                    }
                }
            }
        } elseif (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $this->tokens[$stackPtr]['code']) {
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON !== $this->tokens[$parenthesisOpener]['code'] && \T_FUNCTION !== $this->tokens[$stackPtr]['code']) {
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $this->tokens[$stackPtr]['code'] && 0 === $this->spaces_before_closure_open_paren) {
            ...
            } elseif ((\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE !== $this->tokens[$stackPtr]['code'] || 1 === $this->spaces_before_closure_open_paren) && $stackPtr + 1 === $parenthesisOpener) {
                // Checking this: if[*](...) {}.
        ...
        if (\T_WHITESPACE === $this->tokens[$stackPtr + 1]['code'] && ' ' !== $this->tokens[$stackPtr + 1]['content']) {
            ...
            $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceBeforeOpenParenthesis', $this->tokens[$stackPtr + 1]['content']);
            if (\true === $fix) {
                $this->phpcsFile->fixer->replaceToken($stackPtr + 1, ' ');
            }
        }
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
            if (\T_WHITESPACE !== $this->tokens[$parenthesisOpener + 1]['code']) {
                ...
                $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceAfterOpenParenthesis', $this->tokens[$parenthesisOpener + 1]['content']);
                if (\true === $fix) {
        ...
        if (isset($this->tokens[$parenthesisOpener]['parenthesis_closer'])) {
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_PARENTHESIS !== $this->tokens[$parenthesisOpener + 1]['code']) {
                ...
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser - 1]['code']) {
                    ...
                    if ($this->tokens[$parenthesisCloser]['line'] === $this->tokens[$prevNonEmpty + 1]['line']) {
                        ...
                        $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceBeforeCloseParenthesis', $this->tokens[$parenthesisCloser - 1]['content']);
                        if (\true === $fix) {
                            $this->phpcsFile->fixer->replaceToken($parenthesisCloser - 1, ' ');
                        }
                    }
                }
                if (\T_WHITESPACE !== $this->tokens[$parenthesisCloser + 1]['code'] && !(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON === $this->tokens[$parenthesisCloser + 1]['code'] && (isset($this->tokens[$parenthesisCloser]['parenthesis_owner']) === \false || \in_array($this->tokens[$this->tokens[$parenthesisCloser]['parenthesis_owner']]['code'], array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE), \true))) && (isset($scopeOpener) && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COLON !== $this->tokens[$scopeOpener]['code'])) {
            ...
            if (\T_FUNCTION !== $this->tokens[$stackPtr]['code'] && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE !== $this->tokens[$stackPtr]['code'] && isset($this->tokens[$parenthesisOpener]['parenthesis_owner']) && (isset($scopeOpener) && $this->tokens[$parenthesisCloser]['line'] !== $this->tokens[$scopeOpener]['line'])) {
                ...
                $fix = $this->phpcsFile->addFixableError($error, $stackPtr, 'ExtraSpaceAfterCloseParenthesis', $this->tokens[$parenthesisCloser + 1]['content']);
                if (\true === $fix) {
                    $this->phpcsFile->fixer->replaceToken($parenthesisCloser + 1, ' ');
                }
            }
        ...
        if (\false !== $this->blank_line_check && isset($scopeOpener)) {
            ...
            $ignore = array(\T_FUNCTION => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_OPEN_TAG => \true, \T_CLOSE_TAG => \true, \T_COMMENT => \true);
        ...
        if (isset($this->tokens[$trailingContent]['scope_condition']) && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_CURLY_BRACKET === $this->tokens[$trailingContent]['code']) {
            // Another control structure's closing brace.
            $owner = $this->tokens[$trailingContent]['scope_condition'];
            if (\in_array($this->tokens[$owner]['code'], array(\T_FUNCTION, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE, \T_CLASS, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_ANON_CLASS, \T_INTERFACE, \T_TRAIT), \true)) {
