...
class DisallowInlineTabsSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        $check_tokens = array(\T_WHITESPACE => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_WHITESPACE => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STRING => \true);
        for ($i = $stackPtr + 1; $i < $this->phpcsFile->numTokens; $i++) {
            ...
            $fix = $this->phpcsFile->addFixableError('Spaces must be used for mid-line alignment; tabs are not allowed', $i, 'NonIndentTabsUsed');
            if (\true === $fix) {
                if (isset($this->tokens[$i]['orig_content'])) {
