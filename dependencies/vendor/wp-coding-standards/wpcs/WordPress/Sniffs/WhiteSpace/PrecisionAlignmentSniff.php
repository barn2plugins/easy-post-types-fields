...
class PrecisionAlignmentSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        $check_tokens = array(\T_WHITESPACE => \true, \T_INLINE_HTML => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_WHITESPACE => \true, \T_COMMENT => \true);
        $check_tokens += Tokens::$phpcsCommentTokens;
        for ($i = 0; $i < $this->phpcsFile->numTokens; $i++) {
            if (1 !== $this->tokens[$i]['column']) {
                continue;
            } elseif (isset($check_tokens[$this->tokens[$i]['code']]) === \false || isset($this->tokens[$i + 1]) && \T_WHITESPACE === $this->tokens[$i + 1]['code'] || $this->tokens[$i]['content'] === $this->phpcsFile->eolChar || isset($ignoreAlignmentTokens[$this->tokens[$i]['type']]) || isset($this->tokens[$i + 1]) && isset($ignoreAlignmentTokens[$this->tokens[$i + 1]['type']])) {
                continue;
            }
            $spaces = 0;
            switch ($this->tokens[$i]['type']) {
                ...
                case 'T_DOC_COMMENT_WHITESPACE':
                    ...
                    if (isset($this->tokens[$i + 1]) && (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_STAR === $this->tokens[$i + 1]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_CLOSE_TAG === $this->tokens[$i + 1]['code']) && 0 !== $spaces) {
            ...
            if ($spaces > 0 && !$this->has_whitelist_comment('precision alignment', $i)) {
                $this->phpcsFile->addWarning('Found precision alignment of %s spaces.', $i, 'Found', array($spaces));
            }
        }
