 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\PHP;
...
class IniSetSniff extends AbstractFunctionParameterSniff
{
     ...
     *     );
     */
    protected $whitelisted_options = array('auto_detect_line_endings' => array(), 'highlight.bg' => array(), 'highlight.comment' => array(), 'highlight.default' => array(), 'highlight.html' => array(), 'highlight.keyword' => array(), 'highlight.string' => array(), 'short_open_tag' => array('valid_values' => array('true', '1', 'on')));
    /**
    ...
    public function process_parameters($stackPtr, $group_name, $matched_content, $parameters)
    {
        ...
        if (isset($this->whitelisted_options[$option_name])) {
            ...
            if (!isset($whitelisted_option['valid_values']) || \in_array(\strtolower($option_value), $whitelisted_option['valid_values'], \true)) {
                return;
            }
        }
        if (isset($this->blacklisted_options[$option_name])) {
            $blacklisted_option = $this->blacklisted_options[$option_name];
            if (!isset($blacklisted_option['invalid_values']) || \in_array(\strtolower($option_value), $blacklisted_option['invalid_values'], \true)) {
                $this->phpcsFile->addError('%s(%s, %s) found. %s', $stackPtr, $this->string_to_errorcode($option_name . '_Blacklisted'), array($matched_content, $parameters[1]['raw'], $parameters[2]['raw'], $blacklisted_option['message']));
                return;
            }
        }
        $this->phpcsFile->addWarning('%s(%s, %s) found. Changing configuration values at runtime is strongly discouraged.', $stackPtr, 'Risky', array($matched_content, $parameters[1]['raw'], $parameters[2]['raw']));
    }
}
