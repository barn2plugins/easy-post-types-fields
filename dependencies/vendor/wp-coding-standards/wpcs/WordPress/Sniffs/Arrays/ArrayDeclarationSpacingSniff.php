 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\Sniffs\Arrays;
...
class ArrayDeclarationSpacingSniff extends Sniff
{
     ...
     * @var array
     */
    private $targets = array(\T_ARRAY => \T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY => \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY);
    /**
    ...
    {
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] && $this->is_short_list($stackPtr)) {
        ...
        if (\T_ARRAY === $this->tokens[$stackPtr]['code']) {
            if ($stackPtr + 1 !== $opener) {
                ...
                $nextNonWhitespace = $this->phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, $opener + 1, \true);
         ...
         */
        $nextNonWhitespace = $this->phpcsFile->findNext(\T_WHITESPACE, $opener + 1, $closer + 1, \true);
        if ($nextNonWhitespace === $closer) {
            if ($opener + 1 !== $closer) {
                $fix = $this->phpcsFile->addFixableError('Empty array declaration must have no space between the parentheses', $stackPtr, 'SpaceInEmptyArray');
                if (\true === $fix) {
                    $this->phpcsFile->fixer->beginChangeset();
    ...
    protected function process_single_line_array($stackPtr, $opener, $closer)
    {
        ...
        if (\false !== $array_has_keys) {
            ...
            if (\false === $this->allow_single_item_single_line_associative_arrays && !empty($array_items) || \true === $this->allow_single_item_single_line_associative_arrays && \count($array_items) > 1) {
                ...
                // Reset before doing more detailed check.
                ...
                if (\true === $array_has_keys) {
                    $phrase = 'an';
                    if (\true === $this->allow_single_item_single_line_associative_arrays) {
                        $phrase = 'a multi-item';
                    }
                    $fix = $this->phpcsFile->addFixableError('When %s array uses associative keys, each value should start on a new line.', $closer, 'AssociativeArrayFound', array($phrase));
                    if (\true === $fix) {
                        $this->phpcsFile->fixer->beginChangeset();
                        foreach ($array_items as $item) {
                            /*
                             ...
                             */
                            $first_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $item['start'], $item['end'] + 1, \true);
                            if (\false === $first_non_empty) {
                                continue;
                            }
                            if ($item['start'] <= $first_non_empty - 1 && \T_WHITESPACE === $this->tokens[$first_non_empty - 1]['code']) {
        ...
        if (\T_WHITESPACE !== $this->tokens[$opener + 1]['code']) {
            $fix = $this->phpcsFile->addFixableError('Missing space after array opener.', $opener, 'NoSpaceAfterArrayOpener');
            if (\true === $fix) {
                $this->phpcsFile->fixer->addContent($opener, ' ');
            }
        } elseif (' ' !== $this->tokens[$opener + 1]['content']) {
            $fix = $this->phpcsFile->addFixableError('Expected 1 space after array opener, found %s.', $opener, 'SpaceAfterArrayOpener', array(\strlen($this->tokens[$opener + 1]['content'])));
            if (\true === $fix) {
                $this->phpcsFile->fixer->replaceToken($opener + 1, ' ');
            }
        }
        if (\T_WHITESPACE !== $this->tokens[$closer - 1]['code']) {
            $fix = $this->phpcsFile->addFixableError('Missing space before array closer.', $closer, 'NoSpaceBeforeArrayCloser');
            if (\true === $fix) {
                $this->phpcsFile->fixer->addContentBefore($closer, ' ');
            }
        } elseif (' ' !== $this->tokens[$closer - 1]['content']) {
            $fix = $this->phpcsFile->addFixableError('Expected 1 space before array closer, found %s.', $closer, 'SpaceBeforeArrayCloser', array(\strlen($this->tokens[$closer - 1]['content'])));
            if (\true === $fix) {
    ...
    protected function process_multi_line_array($stackPtr, $opener, $closer)
    {
         ...
         */
        $last_content = $this->phpcsFile->findPrevious(\T_WHITESPACE, $closer - 1, $opener, \true);
        if (\false !== $last_content && $this->tokens[$last_content]['line'] === $this->tokens[$closer]['line']) {
            $fix = $this->phpcsFile->addFixableError('Closing parenthesis of array declaration must be on a new line', $closer, 'CloseBraceNewLine');
            if (\true === $fix) {
                $this->phpcsFile->fixer->beginChangeset();
                if ($last_content < $closer - 1 && \T_WHITESPACE === $this->tokens[$closer - 1]['code']) {
                    // Remove whitespace which would otherwise becoming trailing
                    // (as it gives problems with the fixed file).
                    $this->phpcsFile->fixer->replaceToken($closer - 1, '');
                }
        ...
        foreach ($array_items as $item) {
            ...
            $first_content = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_WHITESPACE), $item['start'], $end_of_this_item, \true);
            ...
            if ($this->tokens[$first_content]['line'] === $this->tokens[$end_of_last_item]['line'] && (\T_COMMENT === $this->tokens[$first_content]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$first_content]['code']]))) {
                ...
                $next = $this->phpcsFile->findNext(array(\T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_WHITESPACE), $end_of_comment + 1, $end_of_this_item, \true);
                if (\false === $next) {
            ...
            if ($this->tokens[$end_of_last_item]['line'] === $this->tokens[$first_content]['line']) {
                $fix = $this->phpcsFile->addFixableError('Each item in a multi-line array must be on a new line', $first_content, 'ArrayItemNoNewLine');
                if (\true === $fix) {
                    $this->phpcsFile->fixer->beginChangeset();
                    if ($end_of_last_item + 1 <= $first_content - 1 && \T_WHITESPACE === $this->tokens[$first_content - 1]['code']) {
                        // Remove whitespace which would otherwise becoming trailing
                        // (as it gives problems with the fixed file).
