...
class ArrayKeySpacingRestrictionsSniff extends Sniff
{
    ...
    {
        return array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        $need_spaces = $this->phpcsFile->findNext(array(\T_CONSTANT_ENCAPSED_STRING, \T_LNUMBER, \T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_MINUS), $stackPtr + 1, $token['bracket_closer'], \true);
        ...
        if (\false !== $need_spaces && (\false === $spaced1 || \false === $spaced2)) {
        ...
        if (\false !== $need_spaces && ($spaced1 || $spaced2)) {
            if ($spaced1) {
                ...
                if (1 !== $length) {
                    ...
                    $data = array($length);
                    $fix = $this->phpcsFile->addFixableError($error, $ptr, 'TooMuchSpaceBeforeKey', $data);
                    if (\true === $fix) {
                        $this->phpcsFile->fixer->beginChangeset();
            ...
            if ($spaced2) {
                $prev_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $token['bracket_closer'] - 1, null, \true);
                ...
                if (1 !== $length) {
                    ...
                    $data = array($length);
                    $fix = $this->phpcsFile->addFixableError($error, $ptr, 'TooMuchSpaceAfterKey', $data);
                    if (\true === $fix) {
                        $this->phpcsFile->fixer->beginChangeset();
                        $this->phpcsFile->fixer->replaceToken($ptr, ' ');
                        for ($i = $ptr + 1; $i < $token['bracket_closer']; $i++) {
