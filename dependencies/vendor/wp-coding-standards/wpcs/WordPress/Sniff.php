 ...
 */
namespace Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress;

use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\PHP_CodeSniffer\Sniffs\Sniff as PHPCS_Sniff;
...
use Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\WordPressCS\WordPress\PHPCSHelper;
/**
 * Represents a PHP_CodeSniffer sniff for sniffing WordPress coding standards.
 *
...
abstract class Sniff implements PHPCS_Sniff
{
     ...
     * @var array
     */
    protected $nonceVerificationFunctions = array('wp_verify_nonce' => \true, 'check_admin_referer' => \true, 'check_ajax_referer' => \true);
    /**
     ...
     * @var array
     */
    protected $unslashingSanitizingFunctions = array('absint' => \true, 'boolval' => \true, 'count' => \true, 'doubleval' => \true, 'floatval' => \true, 'intval' => \true, 'sanitize_key' => \true, 'sizeof' => \true);
    /**
     ...
     * @var array
     */
    protected $unslashingFunctions = array('stripslashes_deep' => \true, 'stripslashes_from_strings_only' => \true, 'wp_unslash' => \true);
    /**
     ...
     * @var array
     */
    protected $typeTestFunctions = array('is_array' => \true, 'is_bool' => \true, 'is_callable' => \true, 'is_countable' => \true, 'is_double' => \true, 'is_float' => \true, 'is_int' => \true, 'is_integer' => \true, 'is_iterable' => \true, 'is_long' => \true, 'is_null' => \true, 'is_numeric' => \true, 'is_object' => \true, 'is_real' => \true, 'is_resource' => \true, 'is_scalar' => \true, 'is_string' => \true);
    /**
     ...
     * @var array
     */
    protected $safe_casts = array(\T_INT_CAST => \true, \T_DOUBLE_CAST => \true, \T_BOOL_CAST => \true, \T_UNSET_CAST => \true);
    /**
     ...
     */
    protected $arrayWalkingFunctions = array('array_map' => 1, 'map_deep' => 2);
    /**
     ...
     */
    protected $arrayCompareFunctions = array('in_array' => \true, 'array_search' => \true, 'array_keys' => 2);
    /**
     ...
     * @var array
     */
    protected $formattingFunctions = array('array_fill' => \true, 'ent2ncr' => \true, 'implode' => \true, 'join' => \true, 'nl2br' => \true, 'sprintf' => \true, 'vsprintf' => \true, 'wp_sprintf' => \true);
    /**
     ...
     * @var array
     */
    protected $printingFunctions = array('_deprecated_argument' => \true, '_deprecated_constructor' => \true, '_deprecated_file' => \true, '_deprecated_function' => \true, '_deprecated_hook' => \true, '_doing_it_wrong' => \true, '_e' => \true, '_ex' => \true, 'printf' => \true, 'trigger_error' => \true, 'user_error' => \true, 'vprintf' => \true, 'wp_die' => \true, 'wp_dropdown_pages' => \true);
    /**
     ...
     * @var array
     */
    protected $SQLEscapingFunctions = array('absint' => \true, 'esc_sql' => \true, 'floatval' => \true, 'intval' => \true, 'like_escape' => \true);
    /**
     ...
     * @var array
     */
    protected $SQLAutoEscapedFunctions = array('count' => \true);
    /**
     ...
     * @var array
     */
    protected $hookInvokeFunctions = array('do_action' => \true, 'do_action_ref_array' => \true, 'do_action_deprecated' => \true, 'apply_filters' => \true, 'apply_filters_ref_array' => \true, 'apply_filters_deprecated' => \true);
    /**
    ...
    protected $test_class_whitelist = array(
        ...
        'Barn2\\Plugin\\Easy_Post_Types_Fields\\Dependencies\\PHPUnit\\Framework\\TestCase' => \true,
     ...
     * @return bool
     */
    protected function addMessage($message, $stackPtr, $is_error = \true, $code = 'Found', $data = array(), $severity = 0)
    {
     ...
     * @return bool
     */
    protected function addFixableMessage($message, $stackPtr, $is_error = \true, $code = 'Found', $data = array(), $severity = 0)
    {
        return $this->throwMessage($message, $stackPtr, $is_error, $code, $data, $severity, \true);
    }
    /**
     * Add a PHPCS message to the output stack as either a warning or an error.
     *
     ...
     * @return bool
     */
    private function throwMessage($message, $stackPtr, $is_error = \true, $code = 'Found', $data = array(), $severity = 0, $fixable = \false)
    {
        $method = 'add';
        if (\true === $fixable) {
            $method .= 'Fixable';
    ...
    {
        $cl_supported_version = \trim(PHPCSHelper::get_config_data('minimum_supported_wp_version'));
        if (!empty($cl_supported_version) && \filter_var($cl_supported_version, \FILTER_VALIDATE_FLOAT) !== \false) {
    ...
    protected function has_whitelist_comment($comment, $stackPtr)
    {
        ...
        if (\true === PHPCSHelper::ignore_annotations($this->phpcsFile)) {
            return \false;
        }
        ...
        $end_of_statement = $this->phpcsFile->findNext(array(\T_CLOSE_TAG, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON), $stackPtr);
        if (\false !== $end_of_statement) {
            ...
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON === $this->tokens[$end_of_statement]['code']) {
                $lastPtr = $this->phpcsFile->findNext(\T_WHITESPACE, $end_of_statement + 1, null, \true);
                ...
                $lastPtr = $this->phpcsFile->findPrevious(\T_WHITESPACE, $end_of_statement - 1, null, \true);
            }
            if ((\T_COMMENT === $this->tokens[$lastPtr]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PHPCS_SET !== $this->tokens[$lastPtr]['code']) && $this->tokens[$lastPtr]['line'] === $this->tokens[$end_of_statement]['line'] && \preg_match($regex, $this->tokens[$lastPtr]['content']) === 1) {
                if (isset($thrown_notices[$filename][$lastPtr]) === \false && isset(Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) === \false) {
                    $this->phpcsFile->addWarning($deprecation_notice, $lastPtr, $deprecation_code, array($this->tokens[$lastPtr]['content']));
                    $thrown_notices[$filename][$lastPtr] = \true;
                }
                return \true;
            }
        }
        ...
        $lastPtr = $this->phpcsFile->findPrevious(\T_WHITESPACE, $end_of_line, null, \true);
        if ((\T_COMMENT === $this->tokens[$lastPtr]['code'] || isset(Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_PHPCS_SET !== $this->tokens[$lastPtr]['code']) && $this->tokens[$lastPtr]['line'] === $this->tokens[$stackPtr]['line'] && \preg_match($regex, $this->tokens[$lastPtr]['content']) === 1) {
            if (isset($thrown_notices[$filename][$lastPtr]) === \false && isset(Tokens::$phpcsCommentTokens[$this->tokens[$lastPtr]['code']]) === \false) {
                $this->phpcsFile->addWarning($deprecation_notice, $lastPtr, $deprecation_code, array($this->tokens[$lastPtr]['content']));
                $thrown_notices[$filename][$lastPtr] = \true;
            }
    ...
    protected function is_test_class($stackPtr)
    {
        ...
        $whitelist = $this->merge_custom_array($this->custom_test_class_whitelist, $this->test_class_whitelist);
        /*
    ...
    {
        static $valid = array(\T_VARIABLE => \true, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET => \true);
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET === $this->tokens[$next_non_empty]['code'] && isset($this->tokens[$next_non_empty]['bracket_closer'])) {
    ...
    protected function has_nonce_check($stackPtr)
    {
        ...
        if (isset($tokens[$stackPtr]['conditions']) === \true) {
            ...
            foreach ($conditions as $tokenPtr => $condition) {
                if (\T_FUNCTION === $condition || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $condition) {
        ...
        if ($this->is_in_isset_or_empty($stackPtr) || $this->is_in_type_test($stackPtr) || $this->is_comparison($stackPtr) || $this->is_in_array_comparison($stackPtr) || $this->is_in_function_call($stackPtr, $this->unslashingFunctions) !== \false || $this->is_only_sanitized($stackPtr)) {
            $allow_nonce_after = \true;
        }
        ...
        $filename = $this->phpcsFile->getFilename();
        if (\is_array($last) && $filename === $last['file'] && $start === $last['start']) {
            if (\false !== $last['nonce_check']) {
                ...
                return \true === $allow_nonce_after || $last['nonce_check'] < $stackPtr;
            } elseif ($end <= $last['end']) {
            ...
            $last = array('file' => $filename, 'start' => $start, 'end' => $end);
        }
        // Loop through the tokens looking for nonce verification functions.
        for ($i = $start; $i < $end; $i++) {
            // Skip over nested closed scope constructs.
            if (\T_FUNCTION === $tokens[$i]['code'] || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE === $tokens[$i]['code'] || isset(Tokens::$ooScopeTokens[$tokens[$i]['code']])) {
            ...
            if (isset($this->nonceVerificationFunctions[$tokens[$i]['content']])) {
                 ...
                 */
                if ($this->is_class_object_call($i) === \true) {
                    continue;
                }
                if ($this->is_token_namespaced($i) === \true) {
                    continue;
                }
                $last['nonce_check'] = $i;
                return \true;
    ...
    protected function is_in_isset_or_empty($stackPtr)
    {
        ...
        $previous_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $open_parenthesis - 1, null, \true, null, \true);
        ...
        $valid_functions = array('array_key_exists' => \true, 'key_exists' => \true);
    ...
    {
        $before = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
        if (\false === $before) {
            return \false;
        }
        if (\T_OBJECT_OPERATOR !== $this->tokens[$before]['code'] && \T_DOUBLE_COLON !== $this->tokens[$before]['code']) {
            return \false;
        }
        return \true;
    }
    ...
    {
        $prev = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true, null, \true);
        if (\false === $prev) {
            return \false;
        }
        ...
        $before_prev = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $prev - 1, null, \true, null, \true);
        if (\false === $before_prev) {
            return \false;
        }
        if (\T_STRING !== $this->tokens[$before_prev]['code'] && \T_NAMESPACE !== $this->tokens[$before_prev]['code']) {
            return \false;
        }
        return \true;
    }
     ...
     */
    protected function is_in_function_call($stackPtr, $valid_functions, $global = \true, $allow_nested = \false)
    {
        if (!isset($this->tokens[$stackPtr]['nested_parenthesis'])) {
            return \false;
        }
        ...
        foreach ($nested_parenthesis as $open => $close) {
            $prev_non_empty = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $open - 1, null, \true, null, \true);
             ...
             */
            if ($this->is_class_object_call($prev_non_empty) === \true) {
                continue;
            }
            if ($this->is_token_namespaced($prev_non_empty) === \true) {
                continue;
            }
            return $prev_non_empty;
        }
     ...
     */
    protected function get_array_access_keys($stackPtr, $all = \true)
    {
        $keys = array();
        ...
        do {
            ...
            $open_bracket = $this->phpcsFile->findNext(Tokens::$emptyTokens, $current + 1, null, \true);
            // If it isn't a bracket, this isn't an array-access.
            if (\false === $open_bracket || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET !== $this->tokens[$open_bracket]['code'] || !isset($this->tokens[$open_bracket]['bracket_closer'])) {
                break;
            }
            $key = $this->phpcsFile->getTokensAsString($open_bracket + 1, $this->tokens[$open_bracket]['bracket_closer'] - $open_bracket - 1);
            $keys[] = \trim($key);
            $current = $this->tokens[$open_bracket]['bracket_closer'];
        } while (isset($this->tokens[$current]) && \true === $all);
        return $keys;
    }
    /**
    ...
    protected function is_validated($stackPtr, $array_keys = array(), $in_condition_only = \false)
    {
        if ($in_condition_only) {
            ...
            if (\false !== $function) {
                ...
                $closure = $this->phpcsFile->getCondition($stackPtr, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE);
        ...
        $targets = array(\T_ISSET => 'construct', \T_EMPTY => 'construct', \T_UNSET => 'construct', \T_STRING => 'function_call', \T_COALESCE => 'coalesce', \T_COALESCE_EQUAL => 'coalesce');
        ...
        for ($i = $scope_start + 1; $i < $scope_end; $i++) {
            ...
            switch ($targets[$this->tokens[$i]['code']]) {
                case 'construct':
                    $issetOpener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true, null, \true);
                    if (\false === $issetOpener || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$issetOpener]['code']) {
                ...
                case 'function_call':
                    ...
                    if ('array_key_exists' !== $this->tokens[$i]['content'] && 'key_exists' !== $this->tokens[$i]['content']) {
                        continue 2;
                    }
                    $next_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true, null, \true);
                    if (\false === $next_non_empty || \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$next_non_empty]['code']) {
                        // Not a function call.
                        continue 2;
                    }
                    if ($this->is_class_object_call($i) === \true) {
                    ...
                    $param2_first_token = $this->phpcsFile->findNext(Tokens::$emptyTokens, $params[2]['start'], $params[2]['end'] + 1, \true);
                    if (\false === $param2_first_token || \T_VARIABLE !== $this->tokens[$param2_first_token]['code'] || $this->tokens[$param2_first_token]['content'] !== $this->tokens[$stackPtr]['content']) {
                        continue 2;
                    }
                    if (!empty($bare_array_keys)) {
                ...
                case 'coalesce':
                    $prev = $i;
                    do {
                        $prev = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $prev - 1, null, \true, null, \true);
                        // Skip over array keys, like $_GET['key']['subkey'].
                        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET === $this->tokens[$prev]['code']) {
     ...
     */
    protected function is_comparison($stackPtr, $include_coalesce = \true)
    {
        ...
        if (isset($this->tokens[$stackPtr]['nested_parenthesis'])) {
            ...
            if (isset($this->tokens[$close_parenthesis]['parenthesis_owner']) && \T_SWITCH === $this->tokens[$this->tokens[$close_parenthesis]['parenthesis_owner']]['code']) {
                return \true;
            }
        }
        // Find the previous non-empty token. We check before the var first because
        // yoda conditions are usually expected.
        $previous_token = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (isset($comparisonTokens[$this->tokens[$previous_token]['code']])) {
            return \true;
        }
        // Maybe the comparison operator is after this.
        $next_token = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true);
        // This might be an opening square bracket in the case of arrays ($var['a']).
        while (\false !== $next_token && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SQUARE_BRACKET === $this->tokens[$next_token]['code']) {
            $next_token = $this->phpcsFile->findNext(Tokens::$emptyTokens, $this->tokens[$next_token]['bracket_closer'] + 1, null, \true);
        }
    ...
    {
        $function_ptr = $this->is_in_function_call($stackPtr, $this->arrayCompareFunctions, \true, \true);
        if (\false === $function_ptr) {
            return \false;
        }
        $function_name = $this->tokens[$function_ptr]['content'];
        if (\true === $this->arrayCompareFunctions[$function_name]) {
            return \true;
        }
    ...
    protected function get_use_type($stackPtr)
    {
        ...
        $next = $this->phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, null, \true);
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS === $this->tokens[$next]['code']) {
            return 'closure';
        }
        // USE keywords for traits.
        $valid_scopes = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_TRAIT' => \true);
    ...
    public function does_function_call_have_parameters($stackPtr)
    {
        ...
        if (\false === \in_array($this->tokens[$stackPtr]['code'], array(\T_STRING, \T_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY), \true)) {
            return \false;
        }
        $next_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $next_next_non_empty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $next_non_empty + 1, $close_parenthesis + 1, \true);
    ...
    public function get_function_call_parameters($stackPtr)
    {
        ...
        if ('T_OPEN_SHORT_ARRAY' === $this->tokens[$stackPtr]['type']) {
            ...
            $opener = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        ...
        $cnt = 1;
        while ($next_comma = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, $this->tokens[$closer]['code'], \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSURE), $next_comma + 1, $closer + 1)) {
            // Ignore anything within short array definition brackets.
            if ('T_OPEN_SHORT_ARRAY' === $this->tokens[$next_comma]['type'] && (isset($this->tokens[$next_comma]['bracket_opener']) && $this->tokens[$next_comma]['bracket_opener'] === $next_comma) && isset($this->tokens[$next_comma]['bracket_closer'])) {
                // Skip forward to the end of the short array definition.
                $next_comma = $this->tokens[$next_comma]['bracket_closer'];
                continue;
            }
            // Skip past closures passed as function parameters.
            if ('T_CLOSURE' === $this->tokens[$next_comma]['type'] && (isset($this->tokens[$next_comma]['scope_condition']) && $this->tokens[$next_comma]['scope_condition'] === $next_comma) && isset($this->tokens[$next_comma]['scope_closer'])) {
                // Skip forward to the end of the closure declaration.
                $next_comma = $this->tokens[$next_comma]['scope_closer'];
                continue;
            }
            // Ignore comma's at a lower nesting level.
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA === $this->tokens[$next_comma]['code'] && isset($this->tokens[$next_comma]['nested_parenthesis']) && \count($this->tokens[$next_comma]['nested_parenthesis']) !== $nestedParenthesisCount) {
                continue;
            }
            ...
            $parameters[$cnt]['raw'] = \trim($this->phpcsFile->getTokensAsString($param_start, $next_comma - $param_start));
            /*
             * Check if there are more tokens before the closing parenthesis.
             * Prevents code like the following from setting a third parameter:
             * functionCall( $param1, $param2, );
             */
            $has_next_param = $this->phpcsFile->findNext(Tokens::$emptyTokens, $next_comma + 1, $closer, \true, null, \true);
    ...
    protected function find_array_open_close($stackPtr)
    {
        ...
        if (isset($opener, $closer)) {
            return array('opener' => $opener, 'closer' => $closer);
        }
        return \false;
    }
    /**
    ...
    protected function find_list_open_close($stackPtr)
    {
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY === $this->tokens[$stackPtr]['code'] && $this->is_short_list($stackPtr) === \true) {
            $opener = $stackPtr;
            $closer = $this->tokens[$stackPtr]['bracket_closer'];
        }
        if (isset($opener, $closer)) {
    ...
    public function determine_namespace($stackPtr)
    {
        ...
        do {
            ...
            if (!empty($this->tokens[$previousNSToken]['scope_condition']) && $this->tokens[$previousNSToken]['scope_condition'] === $previousNSToken) {
                break;
            }
    ...
    public function get_declared_namespace_name($stackPtr)
    {
        ...
        $nextToken = $this->phpcsFile->findNext(Tokens::$emptyTokens, $stackPtr + 1, null, \true, null, \true);
        if (\T_NS_SEPARATOR === $this->tokens[$nextToken]['code']) {
            // Not a namespace declaration, but use of, i.e. `namespace\someFunction();`.
            return \false;
        }
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_CURLY_BRACKET === $this->tokens[$nextToken]['code']) {
            // Declaration for global namespace when using multiple namespaces in a file.
            // I.e.: `namespace {}`.
            return '';
        }
        // Ok, this should be a namespace declaration, so get all the parts together.
        $acceptedTokens = array(\T_STRING => \true, \T_NS_SEPARATOR => \true);
        ...
        while (isset($validTokens[$this->tokens[$nextToken]['code']])) {
            if (isset($acceptedTokens[$this->tokens[$nextToken]['code']])) {
                $namespaceName .= \trim($this->tokens[$nextToken]['content']);
            }
            ++$nextToken;
        }
    ...
    public function is_class_constant($stackPtr)
    {
        ...
        $valid_scopes = array('T_CLASS' => \true, 'T_ANON_CLASS' => \true, 'T_INTERFACE' => \true);
    ...
    public function is_class_property($stackPtr)
    {
        ...
        if (\false !== $scopePtr) {
            ...
            if (empty($this->tokens[$stackPtr]['nested_parenthesis'])) {
                ...
                if ($deepest_open < $scopePtr || isset($this->tokens[$deepest_open]['parenthesis_owner']) === \false || \T_FUNCTION !== $this->tokens[$this->tokens[$deepest_open]['parenthesis_owner']]['code']) {
                    return \true;
                }
            }
        }
    ...
    {
        // Check for wpdb.
        if (\T_VARIABLE === $this->tokens[$stackPtr]['code'] && '$wpdb' !== $this->tokens[$stackPtr]['content'] || \T_STRING === $this->tokens[$stackPtr]['code'] && 'wpdb' !== $this->tokens[$stackPtr]['content']) {
            return \false;
        }
        // Check that this is a method call.
        $is_object_call = $this->phpcsFile->findNext(array(\T_OBJECT_OPERATOR, \T_DOUBLE_COLON), $stackPtr + 1, null, \false, null, \true);
        if (\false === $is_object_call) {
            return \false;
        }
        $methodPtr = $this->phpcsFile->findNext(\T_WHITESPACE, $is_object_call + 1, null, \true, null, \true);
        if (\false === $methodPtr) {
            return \false;
        }
        ...
        $opening_paren = $this->phpcsFile->findNext(\T_WHITESPACE, $methodPtr + 1, null, \true, null, \true);
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS !== $this->tokens[$opening_paren]['code'] || !isset($this->tokens[$opening_paren]['parenthesis_closer'])) {
            return \false;
        }
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA !== $this->tokens[$end]['code']) {
            ++$end;
        }
    ...
    public function is_use_of_global_constant($stackPtr)
    {
        ...
        if (\false !== $next && (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_PARENTHESIS === $this->tokens[$next]['code'] || \T_DOUBLE_COLON === $this->tokens[$next]['code'])) {
            // Function call or declaration.
            return \false;
        }
        // Array of tokens which if found preceding the $stackPtr indicate that a T_STRING is not a global constant.
        $tokens_to_ignore = array('T_NAMESPACE' => \true, 'T_USE' => \true, 'T_CLASS' => \true, 'T_TRAIT' => \true, 'T_INTERFACE' => \true, 'T_EXTENDS' => \true, 'T_IMPLEMENTS' => \true, 'T_NEW' => \true, 'T_FUNCTION' => \true, 'T_DOUBLE_COLON' => \true, 'T_OBJECT_OPERATOR' => \true, 'T_INSTANCEOF' => \true, 'T_INSTEADOF' => \true, 'T_GOTO' => \true, 'T_AS' => \true, 'T_PUBLIC' => \true, 'T_PROTECTED' => \true, 'T_PRIVATE' => \true);
        $prev = $this->phpcsFile->findPrevious(Tokens::$emptyTokens, $stackPtr - 1, null, \true);
        if (\false !== $prev && isset($tokens_to_ignore[$this->tokens[$prev]['type']])) {
            // Not the use of a constant.
            return \false;
        }
        if ($this->is_token_namespaced($stackPtr) === \true) {
            // Namespaced constant of the same name.
            return \false;
        }
        if (\false !== $prev && \T_CONST === $this->tokens[$prev]['code'] && $this->is_class_constant($prev)) {
        ...
        $firstOnLine = $this->phpcsFile->findNext(Tokens::$emptyTokens, $i + 1, null, \true);
        if (\false !== $firstOnLine && \T_USE === $this->tokens[$firstOnLine]['code']) {
            $nextOnLine = $this->phpcsFile->findNext(Tokens::$emptyTokens, $firstOnLine + 1, null, \true);
            if (\false !== $nextOnLine) {
                if (\T_STRING === $this->tokens[$nextOnLine]['code'] && 'const' === $this->tokens[$nextOnLine]['content']) {
    ...
    {
        // Is this one of the tokens this function handles ?
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY !== $this->tokens[$stackPtr]['code'] && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY !== $this->tokens[$stackPtr]['code']) {
            return \false;
        }
        switch ($this->tokens[$stackPtr]['code']) {
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY:
                $opener = $stackPtr;
                $closer = $this->tokens[$stackPtr]['bracket_closer'];
                break;
            case \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SHORT_ARRAY:
                $opener = $this->tokens[$stackPtr]['bracket_opener'];
                $closer = $stackPtr;
                break;
        }
        $nextNonEmpty = $this->phpcsFile->findNext(Tokens::$emptyTokens, $closer + 1, null, \true, null, \true);
        if (\false !== $nextNonEmpty && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL === $this->tokens[$nextNonEmpty]['code']) {
            return \true;
        }
        // Check for short list in foreach, i.e. `foreach($array as [$a, $b])`.
        if ($this->is_foreach_as($stackPtr) === \true) {
            return \true;
        }
        // Maybe this is a short list syntax nested inside another short list syntax ?
        $parentOpen = $opener;
        do {
            $parentOpen = $this->phpcsFile->findPrevious(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY, $parentOpen - 1, null, \false, null, \true);
            if (\false === $parentOpen) {
                return \false;
            }
    ...
    {
        if (\T_LIST !== $this->tokens[$stackPtr]['code'] && \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_OPEN_SHORT_ARRAY !== $this->tokens[$stackPtr]['code']) {
            return array();
        }
        ...
        do {
            ++$current;
            $next_comma = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, $current, $closer);
            if (\false === $next_comma) {
                $next_comma = $closer;
                $last = \true;
            }
    ...
    public static function is_function_deprecated(File $phpcsFile, $stackPtr)
    {
        ...
        $find[] = \T_WHITESPACE;
        $comment_end = $phpcsFile->findPrevious($find, $stackPtr - 1, null, \true);
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOC_COMMENT_CLOSE_TAG !== $tokens[$comment_end]['code']) {
