...
abstract class AbstractArrayAssignmentRestrictionsSniff extends Sniff
{
    ...
    public function register()
    {
        ...
        return array(\T_DOUBLE_ARROW, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET, \T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING);
    }
    /**
    ...
    public function process_token($stackPtr)
    {
        ...
        if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET === $token['code']) {
            $equal = $this->phpcsFile->findNext(\T_WHITESPACE, $stackPtr + 1, null, \true);
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL !== $this->tokens[$equal]['code']) {
                return;
         ...
         * $foo['bar'] = $taz;
         */
        if (\in_array($token['code'], array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET, \T_DOUBLE_ARROW), \true)) {
            $operator = $stackPtr;
            // T_DOUBLE_ARROW.
            if (\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET === $token['code']) {
                $operator = $this->phpcsFile->findNext(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_EQUAL, $stackPtr + 1);
            }
            $keyIdx = $this->phpcsFile->findPrevious(array(\T_WHITESPACE, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_CLOSE_SQUARE_BRACKET), $operator - 1, null, \true);
            if (!\is_numeric($this->tokens[$keyIdx]['content'])) {
                $key = $this->strip_quotes($this->tokens[$keyIdx]['content']);
                $valStart = $this->phpcsFile->findNext(array(\T_WHITESPACE), $operator + 1, null, \true);
                $valEnd = $this->phpcsFile->findNext(array(\Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_COMMA, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_SEMICOLON), $valStart + 1, null, \false, null, \true);
                $val = $this->phpcsFile->getTokensAsString($valStart, $valEnd - $valStart);
                $val = $this->strip_quotes($val);
                $inst[$key][] = array($val, $token['line']);
            }
        } elseif (\in_array($token['code'], array(\T_CONSTANT_ENCAPSED_STRING, \Barn2\Plugin\Easy_Post_Types_Fields\Dependencies\T_DOUBLE_QUOTED_STRING), \true)) {
        ...
        foreach ($this->groups_cache as $groupName => $group) {
            ...
            foreach ($inst as $key => $assignments) {
                foreach ($assignments as $occurance) {
                    ...
                    $this->addMessage($message, $stackPtr, 'error' === $group['type'], $this->string_to_errorcode($groupName . '_' . $key), array($key, $val));
                }
            }
        }
    }
